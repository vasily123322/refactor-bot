...
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, InputMediaPhoto, InputMediaVideo, InputMediaAnimation, FSInputFile
...
class PostingService:
	def __init__(self, bot: Bot, session_or_factory: AsyncSession | async_sessionmaker[AsyncSession]):
		self.bot = bot
		# Поддержка как готовой сессии, так и фабрики сессий
		self.session: Optional[AsyncSession] = session_or_factory if isinstance(session_or_factory, AsyncSession) else None
		self.session_factory: Optional[async_sessionmaker[AsyncSession]] = (
			session_or_factory if not isinstance(session_or_factory, AsyncSession) else None
		)
	...
	async def _ensure_square_video_from_video_note(self, file_id: str) -> str | None:
		...
		try:
			...
			if not os.path.exists(orig_path):
				try:
					logger.info(f"video_note convert: downloading {file_id} -> {orig_path}")
			...
			cmd = [
				...
				"-vf", "scale=640:640:force_original_aspect_ratio=decrease,pad=640:640:(ow-iw)/2:(oh-ih)/2:color=black,format=yuv420p",
			...
			if ret == 0 and os.path.exists(out_path):
				...
				return out_path
			logger.error(f"video_note convert: ffmpeg returned code {ret} for {file_id}")
			return None
		except Exception as e:
	...
	async def send_now(self, channel_id: int, payload: dict) -> list[int] | None:
		try:
			logger.info(f"send_now: target_chat_id={channel_id} type={payload.get('type')} text_len={len(payload.get('text',''))}")
			...
			if fwd_chat and fwd_msg:
				try:
					m = await self._send_with_retry(self.bot.forward_message, chat_id=channel_id, from_chat_id=int(fwd_chat), message_id=int(fwd_msg))
					return [m.message_id]
				except Exception:
	...
	async def schedule(self, channel_id: int, payload: dict, when: datetime | None, dedupe_key: str | None = None) -> PostTask:
		...
		if self.session_factory is not None:
			async with self.session_factory() as session:
				...
				if dedupe_key:
					...
					if dup:
						return dup
				post = PostTask(channel_id=channel_id, payload=payload, dedupe_key=dedupe_key, scheduled_at=when)
				session.add(post)
	...
	def _build_reply_markup(self, payload: dict) -> InlineKeyboardMarkup | None:
		...
		for row in buttons:
			row_btns = []
			for btn in row:
				row_btns.append(InlineKeyboardButton(text=btn.get("text", "Button"), url=btn.get("url")))
	...
	def _build_userbot_reply_markup(self, payload: dict):
		...
		for row in buttons:
			row_btns = []
			for btn in row:
				row_btns.append(PInlineKeyboardButton(text=btn.get("text", "Button"), url=btn.get("url")))
			rows.append(row_btns)
		return PInlineKeyboardMarkup(rows)

	async def _ensure_local_file(self, file_id: str, suggested_ext: str = "mp4") -> str | None:
	...
	async def _dispatch(self, channel_id: int, payload: dict) -> list[int]:
		...
		logger.info(f"dispatch: start type={type_} has_caption={bool(payload.get('caption'))} has_buttons={bool(payload.get('buttons'))}")
		...
		if type_ == "text":
			...
			try:
				...
				if not disable_preview:
					...
					if url:
						...
						m = await self._send_with_retry(self.bot.send_message, chat_id=channel_id, text=text_html, disable_web_page_preview=False, reply_markup=reply_markup, parse_mode=ParseMode.HTML, **common)
					else:
						m = await self._send_with_retry(self.bot.send_message, chat_id=channel_id, text=text, disable_web_page_preview=True, reply_markup=reply_markup, parse_mode=None if entities else ParseMode.MARKDOWN, entities=entities, **common)
				else:
					m = await self._send_with_retry(self.bot.send_message, chat_id=channel_id, text=text, disable_web_page_preview=True, reply_markup=reply_markup, parse_mode=None if entities else ParseMode.MARKDOWN, entities=entities, **common)
			except TelegramBadRequest as e:
				if "can't parse entities" in str(e).lower():
					m = await self._send_with_retry(self.bot.send_message, chat_id=channel_id, text=text, disable_web_page_preview=disable_preview, reply_markup=reply_markup, parse_mode=None, **common)
				else:
					raise
			...
			if paid_on:
				if price_stars <= 0:
					raise TelegramBadRequest("Paid media requires a positive star price")
				...
				try:
					m = await self._send_with_retry(self.bot.send_photo, chat_id=channel_id, photo=photo, caption=cap, has_spoiler=spoiler, show_caption_above_media=show_above, reply_markup=reply_markup, parse_mode=None if caption_entities else ParseMode.MARKDOWN, caption_entities=caption_entities, **common)
				except TelegramBadRequest as e:
					if "can't parse entities" in str(e).lower():
						m = await self._send_with_retry(self.bot.send_photo, chat_id=channel_id, photo=photo, caption=cap, has_spoiler=spoiler, show_caption_above_media=show_above, reply_markup=reply_markup, parse_mode=None, **common)
					else:
						raise
				ids.append(m.message_id)
		elif type_ == "video":
			...
			if paid_on:
				...
				try:
					m = await self._send_with_retry(self.bot.send_video, chat_id=channel_id, video=video, caption=cap, has_spoiler=spoiler, show_caption_above_media=show_above, reply_markup=reply_markup, parse_mode=None if caption_entities else ParseMode.MARKDOWN, caption_entities=caption_entities, **common)
				except TelegramBadRequest as e:
					if "can't parse entities" in str(e).lower():
						m = await self._send_with_retry(self.bot.send_video, chat_id=channel_id, video=video, caption=cap, has_spoiler=spoiler, show_caption_above_media=show_above, reply_markup=reply_markup, parse_mode=None, **common)
					else:
						raise
				ids.append(m.message_id)
			...
			use_userbot = payload.get("use_userbot", True)  # по умолчанию пробуем через userbot
			if pair_mode and use_userbot and userbot is not None:
				try:
					logger.info(f"dispatch: video_note via userbot pair -> download and send")
					# Download video_note locally and send through userbot
					local_path = await self._ensure_local_file(file_id, suggested_ext="mp4") if file_id else None
					if local_path is None:
						raise RuntimeError("userbot pair: failed to prepare local video_note file")
					m1 = await userbot.send_video_note(chat_id=channel_id, video_note=local_path, disable_notification=silent)
					text = payload.get("caption") or ""
					if text or payload.get("buttons"):
						p_markup = self._build_userbot_reply_markup(payload)
						await userbot.send_message(chat_id=channel_id, text=text, reply_markup=p_markup, disable_web_page_preview=True, parse_mode="markdown", disable_notification=silent)
					return [m1.id] if hasattr(m1, "id") else []
				except Exception as e:
					logger.warning(f"dispatch: userbot pair failed, fallback to bot: {e}")
			# Fallbacks: aiogram pair mode or conversion
			if pair_mode:
				logger.info(f"dispatch: video_note pair mode -> send video_note + message via bot")
				m1 = await self._send_with_retry(self.bot.send_video_note, chat_id=channel_id, video_note=file_id, **common)
				if payload.get("caption") or reply_markup:
					await self._send_with_retry(self.bot.send_message, chat_id=channel_id, text=payload.get("caption", ""), reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=True, **common)
				ids.append(m1.message_id)
			else:
				# Convert video_note to square MP4 and send as regular video with caption/buttons
				logger.info(f"dispatch: video_note -> try convert to square video, file_id={file_id}")
				video_path = await self._ensure_square_video_from_video_note(file_id) if file_id else None
				if video_path:
					logger.info(f"dispatch: sending converted video {video_path} with caption/buttons")
					m = await self._send_with_retry(self.bot.send_video, chat_id=channel_id, video=FSInputFile(video_path), caption=payload.get("caption"), reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN, **common)
					ids.append(m.message_id)
				else:
					...
					try:
						...
						if not os.path.exists(orig_path):
							logger.info(f"dispatch: fallback download original video_note to {orig_path}")
							await self.bot.download(file_id, destination=orig_path)
						logger.info(f"dispatch: fallback send as regular video from {orig_path}")
						m = await self._send_with_retry(self.bot.send_video, chat_id=channel_id, video=FSInputFile(orig_path), caption=payload.get("caption"), reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN, **common)
						ids.append(m.message_id)
					except Exception as e:
						logger.exception(f"dispatch: fallback to send_video_note due to error: {e}")
			...
			m = await self.bot.send_animation(chat_id=channel_id, animation=animation, caption=payload.get("caption"), reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN, **common)
			ids.append(m.message_id)
			...
			for item in payload.get("media", []):
				...
				if item.get("type") == "photo":
					...
					media_items.append(InputMediaPhoto(media=photo, caption=caption, parse_mode=None if item.get("caption_entities") else ParseMode.MARKDOWN, caption_entities=item.get("caption_entities")))
				elif item.get("type") == "video":
					video = item.get("file_id") or FSInputFile(item.get("file_path"))
					media_items.append(InputMediaVideo(media=video, caption=caption, parse_mode=None if item.get("caption_entities") else ParseMode.MARKDOWN, caption_entities=item.get("caption_entities")))
				elif item.get("type") == "animation":
					animation = item.get("file_id") or FSInputFile(item.get("file_path"))
					media_items.append(InputMediaAnimation(media=animation, caption=caption, parse_mode=None if item.get("caption_entities") else ParseMode.MARKDOWN, caption_entities=item.get("caption_entities")))
			...
			if paid_on:
				...
				if not paid_media:
					raise TelegramBadRequest("Paid media group supports only photo/video items")
				...
				if payload.get("media"):
					...
					cap_entities = payload.get("media", [])[idx].get("caption_entities") or None
				m = await self._send_with_retry(
					self.bot.send_paid_media,
					chat_id=channel_id,
					star_count=price_stars,
				...
				try:
					msgs = await self._send_with_retry(self.bot.send_media_group, chat_id=channel_id, media=media_items, **common)
				except TelegramBadRequest as e:
					if "can't parse entities" in str(e).lower():
						for it in media_items:
							if isinstance(it, (InputMediaPhoto, InputMediaVideo, InputMediaAnimation)):
								it.parse_mode = None
								it.caption_entities = None
						msgs = await self.bot.send_media_group(chat_id=channel_id, media=media_items, **common)
					else:
						raise
				...
				if reply_markup or payload.get("text"):
					...
					try:
						m2 = await self._send_with_retry(self.bot.send_message, chat_id=channel_id, text=text2, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN, **common)
					except TelegramBadRequest as e:
						if "can't parse entities" in str(e).lower():
							m2 = await self._send_with_retry(self.bot.send_message, chat_id=channel_id, text=text2, reply_markup=reply_markup, parse_mode=None, **common)
						else:
							raise
					ids.append(m2.message_id)
		elif payload.get("type") == "audio":
			m = await self._send_with_retry(self.bot.send_audio, chat_id=channel_id, audio=payload["file_id"], caption=payload.get("caption"), parse_mode="Markdown", **common)
			ids.append(m.message_id)
		elif payload.get("type") == "voice":
			m = await self._send_with_retry(self.bot.send_voice, chat_id=channel_id, voice=payload["file_id"], caption=payload.get("caption"), parse_mode=ParseMode.MARKDOWN, **common)
			ids.append(m.message_id)
		else:
	...
	def apply_autosign_to_payload(payload: dict, autosign_text: str) -> dict:
		...
		def _merge_text(base_text: str, base_entities: list[dict], autosign_md: str, limit: int) -> tuple[str, list[dict], bool]:
			sep = "\n\n" if base_text else ""
		...
		if t == "text":
			...
			final_text, pruned, had_merged = _merge_text(base_text, base_entities, autosign_text, 4096)
		...
		if t in {"photo", "video", "animation", "audio", "voice", "video_note"}:
			...
			final_cap, pruned, had_merged = _merge_text(cap0, base_entities, autosign_text, 1024)
		...
		if t == "album":
			...
			caption_final, pruned, had_merged = _merge_text(cap0, base_entities, autosign_text, 1024)
