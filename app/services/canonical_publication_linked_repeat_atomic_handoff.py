...
class CanonicalPublicationLinkedRepeatAtomicHandoffService:
    ...
    Exact repeat/time compositions select independent read-only parity proofs before the
    legacy transport is retired. The final strict claim remains the authority boundary.
    Combined time+pin+forward has its own route and fact; both narrower facts without the
    combined bit still roll the prepared cutover back atomically.
    """
        ...
        *,
        ...
        try:
            ...
            if task is None or str(task.status) != _CUTOVER_STATUS:
                return await self._rollback_result(safe_publication_id, "conflict", task_id)
            ...
            if publication.schedule_entry_id is None:
                return await self._rollback_result(safe_publication_id, "ineligible", task_id)
            schedule = (
                await self.session.execute(
            ...
            schedule.meta = {**schedule_meta, CUTOVER_META_KEY: deepcopy(cutover_meta)}
            publication.meta = {
                **publication_meta,
            ...
            claim = await CanonicalPublicationRepeatCapabilityClaimService(
                ...
                allow_repeat_time_pin_forward=bool(allow_repeat_time_pin_forward),
