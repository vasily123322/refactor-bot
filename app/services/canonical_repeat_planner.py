...
class CanonicalRepeatPlanner:
    """Pure canonical proof for the next successful repeat occurrence.

    This service never writes ScheduleEntry, Publication or PostTask rows. It proves
    that the canonical domain already contains enough immutable content, repeat,
    delivery and runtime-intent state to calculate the same next fixed-delay slot as
    the legacy scheduler. Persisting or executing the returned plan is a later stage.
    """
    ...
    async def _existing_successor(
        ...
        *,
        ...
        if len(rows) > 1:
            # Multiple canonical occurrences for one exact repeat slot are ambiguous.
            return False
        if not rows:
            return None
    ...
    async def plan_next(
        ...
        *,
        ...
        if source is None:
            return None
        publication, schedule, item, revision, attempt = source
        ...
        # Normal legacy repeat scheduling always starts with source + one interval,
        # then skips any slots that are no longer strictly in the future.
        first_candidate = source_scheduled_at + timedelta(seconds=repeat_seconds)
