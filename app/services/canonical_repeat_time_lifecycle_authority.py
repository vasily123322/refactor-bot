...
class CanonicalRepeatTimeLifecycleAuthority:
    publication_id: int
...
class CanonicalRepeatTimeLifecycleAuthorityService:
    """Provider-free terminal proof for the plain repeat+time lifecycle slice.

    This service deliberately does not authorize Telegram DELETE and does not depend on
    the current non-repeat time deleter. It only proves that one terminal canonical repeat
    occurrence has exact queue-time timer intent plus an exact generated timer runtime.

    The proof is explicit/default-off via `allow_time`. Pin, ordered-forward, views and
    generated/effective queue-time keys remain outside this first time-family slice. A
    later destructive adapter must still converge with the durable per-message #281 action
    ...
    async def lock_and_prove(
        ...
        *,
    ...
    ) -> CanonicalRepeatTimeLifecycleAuthority | None:
        if not bool(allow_time):
            return None
        ...
        if (
            ...
            or not set(runtime_options).issubset(_REPEAT_TIME_RUNTIME_KEYS)
        ):
            return None
        ...
        return CanonicalRepeatTimeLifecycleAuthority(
