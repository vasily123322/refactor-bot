...
class CanonicalPublicationLinkedForwardAtomicHandoffService:
    ...
    async def claim_linked_forward(
        self,
        publication_id: int,
        *,
        holder: str,
        ...
        try:
            ...
            if schedule is None or item is None or revision is None or source_channel is None:
                return await self._rollback_result(
                    safe_publication_id,
                    "ineligible",
                    task_id,
            ...
            if (
                ...
                or int(plan.channel_id) != int(source_channel.id)
            ):
                return await self._rollback_result(
                    safe_publication_id,
                    "ineligible",
            ...
            target_ids = list(parity.forward_channel_ids)
            locked_targets = (
                await self.session.execute(
                    select(Channel)
                    .where(Channel.id.in_(target_ids))
                    .with_for_update()
                )
            ).scalars().all()
            target_by_id = {int(row.id): row for row in locked_targets}
            if len(target_by_id) != len(target_ids):
                return await self._rollback_result(
                    safe_publication_id,
                    "ineligible",
                    task_id,
                )
            for target in parity.forward_targets:
                row = target_by_id.get(int(target.channel_id))
                if row is None:
                    return await self._rollback_result(
                        safe_publication_id,
                        "ineligible",
                ...
                try:
                    telegram_chat_id = int(row.tg_chat_id)
                except (TypeError, ValueError, OverflowError):
                    return await self._rollback_result(
                        safe_publication_id,
                        "ineligible",
                ...
                if telegram_chat_id != int(target.telegram_chat_id):
                    return await self._rollback_result(
                        safe_publication_id,
                        "ineligible",
                        task_id,
