...
class ExternalBotsManager:
	...
	async def _poll(self, dp: Dispatcher, bot: Bot, ext_id: int) -> None:
		try:
			...
			# Попробуем мягкий авто-рескатуринг с бэкофом, если бот всё ещё активен
			try:
				async with AsyncSessionLocal() as session:
					repo = ExternalBotsRepo(session)
					ext = await repo.get_by_id(ext_id)
		...
		while True:
			try:
				...
				async with a_sync() as session:
					...
					res = await session.execute(select(ChannelBot).where(ChannelBot.external_bot_id == ext_id, ChannelBot.mode == 1))
					...
					for cb in channels:
						...
						for jr in items:
							...
							if jr.challenge_type and (jr.attempts_left is not None) and (jr.attempts_left > 0):
								continue
							...
							if not bot:
								continue
							with contextlib.suppress(Exception):
								await bot.approve_chat_join_request(chat_id=int(ch.tg_chat_id), user_id=jr.user_id)
						with contextlib.suppress(Exception):
