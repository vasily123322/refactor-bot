...
class CanonicalPublicationLinkedRepeatTimeForwardParityService:
    """Read-only exact parity for ordered repeat+time+forward composition.

    Plain repeat+time remains unchanged and rejects forward. This proof verifies exact
    canonical/legacy ordered forward targets, strips only that already-proven effect into
    a temporary read-only view, delegates every other repeat/time identity check to the
    plain parity service, then restores the ordered targets in the immutable proof.

    No target snapshot, transport cutover, provider call or destructive authority is
    created here. Pin, views and combined time+pin+forward remain outside this stage.
    """
