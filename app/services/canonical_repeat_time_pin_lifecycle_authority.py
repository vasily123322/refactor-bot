...
class CanonicalRepeatTimePinLifecycleAuthority:
    publication_id: int
...
class CanonicalRepeatTimePinLifecycleAuthorityService:
    """Provider-free terminal proof for exact repeat+time+pin lifecycle state."""
    ...
    async def lock_and_prove(
        ...
        publication_id: int,
        *,
        allow_time_pin: bool = False,
    ) -> CanonicalRepeatTimePinLifecycleAuthority | None:
        if not bool(allow_time_pin):
            return None
        ...
        return CanonicalRepeatTimePinLifecycleAuthority(
