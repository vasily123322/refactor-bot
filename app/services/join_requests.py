...
class JoinRequestsService:
    ...
    async def _write_modlog(self, channel_id: int, action: str, user_id: int, meta: dict | None = None) -> None:
        try:
    ...
    async def handle_join_request(self, external_bot_id: int, event: ChatJoinRequest) -> None:
        ...
        full_name = " ".join([p for p in [getattr(user, "first_name", None), getattr(user, "last_name", None)] if p])

        ch_repo = ChannelsRepo(self.session)
        cb_repo = ChannelBotsRepo(self.session)
        subs_repo = SubscribersRepo(self.session)
        ...
        require_mode = int(meta.get("require_dm_mode", 1))  # 0 off, 1 simple, 2 captcha, 3 keyword
        ...
        if (wl_u or wl_i) and not ((uname and uname in wl_u) or (user_id in wl_i)):
            await self.session.add(JoinRequest(channel_id=ch.id, user_id=user_id, status="rejected"))
            await self.session.commit()
            await self._write_modlog(ch.id, "reject", user_id, {"reason": "whitelist_miss"})
            return
        ...
        if mode == 0:
            ...
            if require_mode != 0:
                ...
                if require_mode == 1:
                    ...
                    res2 = await self.session.execute(
                        _select(JoinRequest).where(JoinRequest.channel_id == ch.id, JoinRequest.user_id == user_id, JoinRequest.status == "pending")
                    )
                    ...
                    kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text=label)]], resize_keyboard=True, one_time_keyboard=True, is_persistent=False)
                    ...
                    a = pp.get("a")
                    b = pp.get("b")
                    text = (cfg.get("captcha_prompt") or "Решите пример: {a} + {b} = ?").format(a=a, b=b)
                    with suppress(Exception):
                        await event.bot.send_message(user_id, text)
                elif require_mode == 3:
                    word = (cfg.get("keyword_word") or "START")
                    text = (cfg.get("keyword_prompt") or "Отправьте слово: {w}").format(w=word)
                    with suppress(Exception):
                        await event.bot.send_message(user_id, text)
                return
			...
			with suppress(Exception):
				inv = None
				try:
					inv = getattr(getattr(event, "invite_link", None), "invite_link", None)
				except Exception:
					inv = None
				await save_subscriber_preference(self.session, channel_id=ch.id, user_id=user_id, username=username, full_name=full_name, tag=(f"inv:{inv}" if inv else None))
        ...
        if bool(meta.get("anti_raid_enabled", False)):
            ...
            res = await self.session.execute(
                _select(JoinRequest).where(
                    ...
                    JoinRequest.created_at > (datetime.now(timezone.utc) - timedelta(minutes=1)),
                )
            )
