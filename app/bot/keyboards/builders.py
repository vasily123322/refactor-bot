...
def build_media_menu_kb(pos_label: str, spoiler_checked: bool, *, paid_on: bool | None = None, is_album: bool = False, price: int | None = None) -> InlineKeyboardMarkup:
    label_pos = "Расположение: сверху" if pos_label != "bottom" else "Расположение: снизу"
    label_sp = ("✅ Спойлер" if spoiler_checked else "☑️ Спойлер")
    label_paid = ("✅ Платный пост" if paid_on else "☑️ Платный пост")
    price_row = [InlineKeyboardButton(text=(f"Цена за пост: {price} ⭐" if (paid_on and price) else "Цена за пост"), callback_data=CB.MEDIA_PAID_PRICE)] if paid_on else []
    rows = [
        ...
        (None if paid_on else [InlineKeyboardButton(text=label_sp, callback_data=CB.MEDIA_SPOILER_TOGGLE)]),
...
def build_preview_menu_kb(show_above: bool, *, show_enabled: bool = True) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="Преобразовать в медиа", callback_data=CB.PREVIEW_CLEAR)],
        [InlineKeyboardButton(text="← Назад", callback_data=CB.POST_BACK)],
    ]
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def build_back_kb() -> InlineKeyboardMarkup:
    from app.core.callbacks import CB
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="← Назад", callback_data=CB.POST_BACK)]])
...
def build_menu(title_rows: list[tuple[str, str]], *, back_to: str | None = None) -> InlineKeyboardMarkup:
    """Построить простое меню из (label, callback_data), опционально добавить назад."""
    rows = [[InlineKeyboardButton(text=label, callback_data=cb)] for label, cb in title_rows]
