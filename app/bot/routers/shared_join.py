    ...
    async def on_chat_member(update: ChatMemberUpdated):
        try:
            ...
            if not chat_id or not new or not old:
                return
            if (
                str(getattr(old, "status", "")) in ("member", "administrator")
                and str(getattr(new, "status", "")) == "left"
            ):
                ...
                if int(getattr(cb, "mode", 0)) == 0:
                    with suppress(Exception):
                        await callback.bot.approve_chat_join_request(
                            chat_id=int(ch.tg_chat_id), user_id=user_id
                        )
                    # upsert подписчика
            ...
            async with AsyncSessionLocal() as session:
                ...
                res = await session.execute(
                    select(JoinRequest).where(
                        ...
                        (JoinRequest.expires_at.is_(None))
                ...
                for jr in items:
                    ...
                    if cb and int(getattr(cb, "mode", 0)) == 0:
                        ...
                        if ch:
                            ...
                            from app.repositories.subscribers import (
                                SubscribersRepo as _Subs,
                            )

                            _Subs(session)
                            from app.services.subscribers import (
                                save_subscriber_preference,
                            )

                            with suppress(Exception):
                                await save_subscriber_preference(
                                    session,
                                    channel_id=jr.channel_id,
                                    user_id=user_id,
                                    username=getattr(user, "username", None),
                                    full_name=getattr(user, "full_name", None),
                                )
                            ...
                            try:
                                ...
                                utm = pp.get("utm")
                                if not utm:
                                    cfg = await cb_repo.get_require_dm_config(
                                        jr.channel_id
                                    )
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                if not jr:
                    ...
                    require_mode = int(meta.get("require_dm_mode", 0))
                    pp = {}
                    if require_mode == 2:
                        import random
                ...
                if require_mode == 1:
                    ...
                    if int(getattr(cb, "mode", 0)) == 0:
                        ...
                        with suppress(Exception):
                            await message.answer(
                                "Спасибо! Ожидайте одобрения модератора."
                            )
                    with suppress(Exception):
                        await message.answer(
                            text,
                            reply_markup=InlineKeyboardMarkup(
                                inline_keyboard=[
                                    [
                                        InlineKeyboardButton(
                                            text="OK", callback_data=f"dm_solve:{cid}"
                                        )
                                    ]
                                ]
                            ),
                    ...
                    msg = (cfg.get("keyword_prompt") or "Отправьте слово: {w}").format(
                        w=word
                    )
                    with suppress(Exception):
                        await message.answer(msg)
