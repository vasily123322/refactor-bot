...
def _is_admin(message: Message) -> bool:
	try:
		admin_id = int(settings.admin_user_id) if settings.admin_user_id is not None else None
...
async def cmd_set_log_channel(message: Message):
	...
	if not chat_id:
		return await message.answer("Не удалось определить канал/чат. Укажите /set_log_channel @username или /set_log_channel -100..., либо ответьте командой на пересланное сообщение из нужного канала/чата.")
	# Проверим тип чата (должен быть channel/supergroup/group)
	try:
		info2 = await tg_bot.get_chat(chat_id)
		ctype = str(getattr(info2, "type", ""))
		if ctype == "private":
			return await message.answer("Нельзя назначить личный чат в качестве лог-канала. Укажите канал, супергруппу или группу.")
	except Exception:
		pass
