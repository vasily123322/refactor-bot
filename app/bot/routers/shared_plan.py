...
def build_calendar_kb(channel_id: int, focus: datetime, selected: datetime | None) -> InlineKeyboardMarkup:
	...
	rows = []
	rows.append([
		InlineKeyboardButton(text=f"← {month_name_ru(prev_month)}", callback_data=f"cp_month_prev:{channel_id}:{prev_month_year:04d}-{prev_month:02d}-01"),
		InlineKeyboardButton(text=f"{month_name_ru(month)}", callback_data="cp_ignore"),
		InlineKeyboardButton(text=f"{month_name_ru(next_month)} →", callback_data=f"cp_month_next:{channel_id}:{next_month_year:04d}-{next_month:02d}-01"),
	])
...
async def render_calendar(callback: CallbackQuery, channel_id: int, focus_date: datetime, selected_date: datetime | None) -> None:
