...
from app.bot.keyboards.posting import post_actions, settings_menu_kb, _format_duration_label
...
from app.bot.routers.shared import parse_duration_free as _parse_duration_free
...
async def cb_post_timer_menu(callback: CallbackQuery, state: FSMContext):
    ...
    cur_label = _format_duration_label(cur_sec)
    ...
    rows.append([InlineKeyboardButton(text="👁 Удаление по просмотрам", callback_data=CB.POST_VIEW_MENU)])
    # Ряд с «нет» и минутами
    rows.append([
        InlineKeyboardButton(text="нет", callback_data=CB.POST_TIMER_CLEAR),
        InlineKeyboardButton(text="5 мин", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{5*60}"),
        InlineKeyboardButton(text="15 мин", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{15*60}"),
        InlineKeyboardButton(text="30 мин", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{30*60}"),
    ])
    # Часы: 1,2,4
    rows.append([
        InlineKeyboardButton(text="1 ч", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{1*3600}"),
        InlineKeyboardButton(text="2 ч", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{2*3600}"),
        InlineKeyboardButton(text="4 ч", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{4*3600}"),
    ])
    # Часы/дни: 12ч, 18ч, 1д
    rows.append([
        InlineKeyboardButton(text="12 ч", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{12*3600}"),
        InlineKeyboardButton(text="18 ч", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{18*3600}"),
        InlineKeyboardButton(text="1 д", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{1*24*3600}"),
    ])
    # Дни: 2,3,4
    rows.append([
        InlineKeyboardButton(text="2 д", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{2*24*3600}"),
        InlineKeyboardButton(text="3 д", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{3*24*3600}"),
        InlineKeyboardButton(text="4 д", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{4*24*3600}"),
    ])
    # Дни: 5,7
    rows.append([
        InlineKeyboardButton(text="5 д", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{5*24*3600}"),
        InlineKeyboardButton(text="7 д", callback_data=f"{CB.POST_TIMER_PRESET_PREFIX}{7*24*3600}"),
    ])
    # Отчёт-тумблер и управление
    rows.append([InlineKeyboardButton(text=("✅ Прислать отчёт об удалении" if report_on else "☑️ Прислать отчёт об удалении"), callback_data=CB.POST_TIMER_REPORT)])
    rows.append([InlineKeyboardButton(text="← Назад", callback_data=CB.POST_SETTINGS_BACK)])
    with suppress(TelegramBadRequest):
        await callback.message.edit_text(title, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    # пометим подменю настроек и запомним id сообщения меню автудаления
    with suppress(Exception):
        await state.update_data(ui_submenu="autodel", autodel_menu_msg_id=callback.message.message_id)
...
async def cb_post_view_menu(callback: CallbackQuery, state: FSMContext):
    ...
    presets = [0, 200, 500, 1000, 2000, 3000, 5000, 10000, 20000, 30000, 50000, 100000, 200000, 300000]
    rows: list[list[InlineKeyboardButton]] = []
    # Переключатель обратно на таймер
    rows.append([InlineKeyboardButton(text="🕒 Таймер удаления", callback_data=CB.POST_SETTINGS_TIMER)])
    # Первый ряд: нет и мелкие значения
    rows.append([
        InlineKeyboardButton(text="нет", callback_data=f"{CB.POST_VIEW_PRESET_PREFIX}0"),
        InlineKeyboardButton(text="200", callback_data=f"{CB.POST_VIEW_PRESET_PREFIX}200"),
        InlineKeyboardButton(text="500", callback_data=f"{CB.POST_VIEW_PRESET_PREFIX}500"),
        InlineKeyboardButton(text="1к", callback_data=f"{CB.POST_VIEW_PRESET_PREFIX}1000"),
    ])
    ...
    for line in grid:
        rows.append([InlineKeyboardButton(text=_label(v), callback_data=f"{CB.POST_VIEW_PRESET_PREFIX}{v}") for v in line])
    rows.append([InlineKeyboardButton(text=("✅ Прислать отчёт об удалении" if report_on else "☑️ Прислать отчёт об удалении"), callback_data=CB.POST_VIEW_REPORT)])
    rows.append([InlineKeyboardButton(text="← Назад", callback_data=CB.POST_SETTINGS_BACK)])
    with suppress(TelegramBadRequest):
        await callback.message.edit_text(title, reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    with suppress(Exception):
...
async def cb_post_view_preset(callback: CallbackQuery, state: FSMContext):
    ...
    if post_id and date_iso:
        try:
            ...
            pass
        return await callback.answer("👁 Удаление по просмотрам: установлено", show_alert=True)
...
async def cb_post_view_clear(callback: CallbackQuery, state: FSMContext):
    ...
    if post_id and date_iso:
        try:
            ...
            pass
        return await callback.answer("👁 Удаление по просмотрам: очищено", show_alert=True)
...
async def _cb_timer_short_preset_gate(callback: CallbackQuery, state: FSMContext, sec: int) -> bool:
    try:
        data = await state.get_data()
        ...
        if chan_id:
            async with AsyncSessionLocal() as s:
                ...
                if ch:
                    ...
                    is_pro = bool(getattr(owner, "is_premium", False)) if owner else False
        if (not is_pro) and int(sec) < 5 * 3600:
            admin_url = f"https://t.me/{settings.admin_username}" if settings.admin_username else "https://t.me/vasilyiusii"
            ch_title = None
            if chan_id:
                try:
            ...
            try:
                async with AsyncSessionLocal() as _slog:
                    ...
                    if log_chat_id:
                        ...
                        with suppress(Exception):
                            await tg_bot.send_message(int(log_chat_id), txt, disable_web_page_preview=True)
            except Exception:
                pass
            await callback.answer()
            return True
        ...
        if chan_id:
            async with AsyncSessionLocal() as s:
                ...
                if ch:
                    owner = await s.get(Client, int(getattr(ch, "owner_id", 0)))
                    is_pro = bool(getattr(owner, "is_premium", False)) if owner else False
                    ch_title = getattr(ch, "title", None) or str(getattr(ch, "tg_chat_id", ""))
        return chan_id, is_pro, ch_title
    except Exception:
        return 0, False, None


async def _pro_gate_show_ui_and_log(callback: CallbackQuery, chan_id: int, ch_title: str | None, main_text: str, feature: str, sec: int | None = None) -> None:
    admin_url = f"https://t.me/{settings.admin_username}" if settings.admin_username else "https://t.me/vasilyiusii"
    text_tpl = (
        f"Здравствуйте, пишу по поводу подписки Pro. Хочу приобрести. Канал: {ch_title or chan_id} (ID: {chan_id}). "
        f"Мой ник: @{callback.from_user.username or ''}."
...
async def cb_post_timer_preset(callback: CallbackQuery, state: FSMContext):
    ...
    kb = settings_menu_kb(
        ...
        views_value=int((dict((await state.get_data()).get("payload") or {}).get("autodelete_views") or 0)),
...
async def cb_post_toggle_notify(callback: CallbackQuery, state: FSMContext):
	...
	if data.get("ui_submenu") == "settings":
		...
		with suppress(TelegramBadRequest):
			...
			await tg_bot.edit_message_text(chat_id=callback.message.chat.id, message_id=int(pid), text="⚙️ Настройки публикации", reply_markup=kb)
...
async def cb_post_repeat_menu(callback: CallbackQuery, state: FSMContext):
    ...
    rows.append([
        ...
        InlineKeyboardButton(text="2 ч", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{2*3600}"),
        InlineKeyboardButton(text="4 ч", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{4*3600}"),
        InlineKeyboardButton(text="6 ч", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{6*3600}"),
    ])
    # Часы/дни
    rows.append([
        InlineKeyboardButton(text="12 ч", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{12*3600}"),
        InlineKeyboardButton(text="18 ч", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{18*3600}"),
        InlineKeyboardButton(text="1 д", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{1*24*3600}"),
    ])
    rows.append([
        InlineKeyboardButton(text="2 д", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{2*24*3600}"),
        InlineKeyboardButton(text="3 д", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{3*24*3600}"),
        InlineKeyboardButton(text="4 д", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{4*24*3600}"),
    ])
    rows.append([
        InlineKeyboardButton(text="5 д", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{5*24*3600}"),
        InlineKeyboardButton(text="7 д", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{7*24*3600}"),
        InlineKeyboardButton(text="10 д", callback_data=f"{CB.POST_REPEAT_PRESET_PREFIX}{10*24*3600}"),
    ])
    rows.append([InlineKeyboardButton(text="← Назад", callback_data=CB.POST_SETTINGS_BACK)])
    kb = InlineKeyboardMarkup(inline_keyboard=rows)
    with suppress(TelegramBadRequest):
    ...
    with suppress(Exception):
        await state.update_data(ui_submenu="repeat", repeat_menu_msg_id=callback.message.message_id)
    from app.bot.fsm.states import PostFSM
    await state.set_state(PostFSM.defer_time_input)
...
async def on_edit_post_pick(message: Message, state: FSMContext):
	...
	payload = _build_payload_from_message(message, prefer_html_caption=True, include_entities=False)
	if not payload:
	...
	try:
		...
		pass
	await state.update_data(channel_id=chan_id or 0, edit_chat_id=orig_chat_id, edit_msg_id=orig_msg_id, payload=payload, notify_on=True, autosign_on=autosign_default, pin_on=False, comments_on=True, is_draft=False)
	from app.bot.routers.main import _send_preview_message  # локальный импорт, чтобы избегать циклов
...
async def cb_post_add_button(callback: CallbackQuery, state: FSMContext):
	...
	if btns:
		for i, row in enumerate(btns):
			...
			for j, b in enumerate(row):
				r.append(InlineKeyboardButton(text=f"✖ {b.get('text','')}", callback_data=f"{CB.POST_BUTTON_REMOVE_PREFIX}{i}:{j}"))
			rows_kb.append(r)
		rows_kb.append([InlineKeyboardButton(text="Удалить все", callback_data=CB.POST_DELETE_BUTTONS)])
	...
	with suppress(TelegramBadRequest):
		...
		try:
			await tg_bot.edit_message_text(chat_id=callback.message.chat.id, message_id=int(pid), text=text_html, parse_mode="HTML", disable_web_page_preview=True, reply_markup=kb)
		except TelegramBadRequest:
			with suppress(TelegramBadRequest):
				await tg_bot.edit_message_caption(chat_id=callback.message.chat.id, message_id=int(pid), caption=text_html, parse_mode="HTML", reply_markup=kb)
	with suppress(Exception):
		...
		if prev_id:
			with suppress(TelegramBadRequest):
				if payload.get("type") in {"text","album"}:
					await tg_bot.edit_message_text(chat_id=callback.message.chat.id, message_id=prev_id, text=instruction, parse_mode="Markdown", disable_web_page_preview=True)
				elif payload.get("type") in {"photo","video","animation","audio"}:
					await tg_bot.edit_message_caption(chat_id=callback.message.chat.id, message_id=prev_id, caption=instruction, parse_mode="Markdown")
			with suppress(TelegramBadRequest):
				await tg_bot.edit_message_reply_markup(chat_id=callback.message.chat.id, message_id=prev_id, reply_markup=kb)
	await callback.answer()
...
async def _remove_button_from_payload(state: FSMContext, payload: dict, i: int, j: int) -> list[list[dict]] | None:
...
async def _buttons_mode_maybe_redraw(callback: CallbackQuery, state: FSMContext, payload: dict) -> None:
	cur = await state.get_state()
	if cur != PostFSM.buttons.state:
		return
	...
	if not btns2:
		...
		if prev_id2:
			...
			await tg_bot.edit_message_reply_markup(chat_id=callback.message.chat.id, message_id=prev_id2, reply_markup=kb)
		await state.update_data(buttons_prompt_ids=[])
		return
	from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
	remove_kb_rows = []
...
def _apply_new_text_to_payload(payload: dict, message: Message, new_text: str) -> dict:
	...
	if pl.get("type") == "text":
		...
		if getattr(message, "caption_entities", None):
			pl["caption_entities"] = [e.model_dump() for e in (message.caption_entities or [])]
...
async def _resend_preview_if_present_after_text(message: Message, state: FSMContext, payload: dict, data: dict) -> None:
	prev_id = data.get("preview_msg_id")
	if not prev_id:
		return
	with suppress(TelegramBadRequest):
...
async def _resend_preview_after_buttons(message: Message, state: FSMContext, payload: dict, data: dict) -> None:
	data2 = await state.get_data()
...
async def cb_post_add_media(callback: CallbackQuery, state: FSMContext):
	...
	data = await state.get_data()
	if data.get("edit_chat_id") and data.get("edit_msg_id") and not bool(data.get("is_draft", False)):
	...
	if pl.get("type") == "text":
		try:
			...
			with suppress(TelegramBadRequest):
				await tg_bot.edit_message_text(chat_id=callback.message.chat.id, message_id=int(prev_id), text="🖼 <b>Превью</b>\n\nОтправьте боту ссылку для отображения превью. Медиа не поддерживается.", parse_mode="HTML", disable_web_page_preview=True)
			with suppress(TelegramBadRequest):
...
async def _cb_back_restore_from_media_submenu(callback: CallbackQuery, state: FSMContext) -> bool:
...
async def _cb_back_from_preview_state(callback: CallbackQuery, state: FSMContext) -> bool:
...
async def _cb_back_from_buttons_state(callback: CallbackQuery, state: FSMContext) -> bool:
	data = await state.get_data()
	prev_id = data.get("preview_msg_id") or callback.message.message_id
	kb = await _build_preview_kb(data)
	instr_html = "🖊️ <b>Редактор поста</b>\n\nЕсли нужно изменить текст — просто пришлите новый текст.\nЧтобы добавить фото или видео, отправьте их боту."
...
async def _cb_back_from_content_state(callback: CallbackQuery, state: FSMContext) -> bool:
	from app.bot.routers.main import _send_preview_message
	...
	if prev_id_back:
		with suppress(TelegramBadRequest):
			await tg_bot.delete_message(chat_id=callback.message.chat.id, message_id=prev_id_back)
	preview2 = await _send_preview_message(
		...
		edit_mode=(data_back.get("edit_chat_id") is not None and data_back.get("edit_msg_id") is not None and not bool(data_back.get("is_draft", False))),
		has_buttons=False,
		state=state,
	)
...
async def _cb_back_close_media_menu(callback: CallbackQuery, state: FSMContext) -> bool:
	...
	with suppress(TelegramBadRequest):
		await tg_bot.edit_message_reply_markup(chat_id=callback.message.chat.id, message_id=callback.message.message_id, reply_markup=kb)
	await state.set_state(PostFSM.preview)
	with suppress(TelegramBadRequest):
		await callback.answer()
	return True
...
async def cb_media_menu(callback: CallbackQuery, state: FSMContext):
	...
	kb = build_media_menu_kb(pos, spoiler, paid_on=paid_on, is_album=is_album, price=(star_price or None))
...
async def cb_media_pos_toggle(callback: CallbackQuery, state: FSMContext):
	...
	try:
		if bool(pl.get("media_paid_on", False)) and int(pl.get("media_paid_price") or 0) > 0 and pl.get("type") in {"photo","video","album"}:
			...
			if media_id:
				with suppress(TelegramBadRequest):
					await tg_bot.delete_message(chat_id=callback.message.chat.id, message_id=int(media_id))
			for mid in (data.get("preview_album_ids") or []):
				with suppress(TelegramBadRequest):
					await tg_bot.delete_message(chat_id=callback.message.chat.id, message_id=int(mid))
			await state.update_data(preview_album_ids=[])
			# Отправим новое платное медиа
			paid_msg_id = None
			try:
			...
			if pl.get("type") == "photo" and InputPaidMediaPhoto is not None:
				pm = await tg_bot.send_paid_media(chat_id=callback.message.chat.id, star_count=int(pl.get("media_paid_price") or 0), media=[InputPaidMediaPhoto(media=pl.get("file_id"))], caption=(pl.get("caption") or None), parse_mode=(None if pl.get("caption_entities") else "Markdown"), caption_entities=pl.get("caption_entities"), show_caption_above_media=show_above)
				paid_msg_id = getattr(pm, "message_id", None)
			elif pl.get("type") == "video" and InputPaidMediaVideo is not None:
				pm = await tg_bot.send_paid_media(chat_id=callback.message.chat.id, star_count=int(pl.get("media_paid_price") or 0), media=[InputPaidMediaVideo(media=pl.get("file_id"))], caption=(pl.get("caption") or None), parse_mode=(None if pl.get("caption_entities") else "Markdown"), caption_entities=pl.get("caption_entities"), show_caption_above_media=show_above)
				paid_msg_id = getattr(pm, "message_id", None)
				...
				for it2 in items:
					if (it2.get("caption") or "").strip():
						...
						cent_to_use = it2.get("caption_entities")
				if paid_media:
					pm = await tg_bot.send_paid_media(chat_id=callback.message.chat.id, star_count=int(pl.get("media_paid_price") or 0), media=paid_media, caption=cap_to_use, parse_mode=(None if cent_to_use else "Markdown"), caption_entities=cent_to_use, show_caption_above_media=show_above)
					paid_msg_id = getattr(pm, "message_id", None)
			if paid_msg_id:
				await state.update_data(preview_media_id=int(paid_msg_id))
			# Под ним карточку «Медиа»
			from app.bot.keyboards.builders import build_media_menu_kb
			kb = build_media_menu_kb(pl.get("media_pos", "top"), bool(pl.get("media_spoiler", False)), paid_on=True, is_album=(pl.get("type")=="album"), price=int(pl.get("media_paid_price") or 0))
			header_html = "<b>Медиа</b>\n\nНастройте, как будет выглядеть медиа-файл в посте."
			m_card = await callback.message.answer(header_html, reply_markup=kb, parse_mode="HTML")
			await state.update_data(preview_msg_id=m_card.message_id, ui_submenu="media")
			await state.set_state(PostFSM.preview)
			await callback.answer()
			return
...
async def cb_media_spoiler_toggle(callback: CallbackQuery, state: FSMContext):
	...
	if bool(pl.get("media_paid_on", False)) and int(pl.get("media_paid_price") or 0) > 0:
		with suppress(TelegramBadRequest):
			await callback.answer()
		return
...
async def cb_media_paid_toggle(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        from app.bot.routers.main import _send_preview_message  # локальный импорт, чтобы избежать циклов
        st2 = await state.get_data()
        # Удалим старый предпросмотр и альбом, если был
        # Удалим карточку и медиа, если известны их id
        prev_id = st2.get("preview_msg_id") or getattr(callback.message, "message_id", None)
        media_id = st2.get("preview_media_id")
        if prev_id:
            with suppress(TelegramBadRequest):
                await tg_bot.delete_message(chat_id=callback.message.chat.id, message_id=int(prev_id))
        ...
        edit_mode = (st2.get("edit_chat_id") is not None and st2.get("edit_msg_id") is not None and not is_draft)
...
async def cb_media_paid_price(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pid = st.get("preview_msg_id") or getattr(callback.message, "message_id", None)
        if pid:
            text_price = "🪙 Цена за пост\n\nОтправьте боту стоимость поста в звёздах (1–2500)."
            try:
                await tg_bot.edit_message_text(chat_id=callback.message.chat.id, message_id=int(pid), text=text_price, parse_mode="HTML", reply_markup=kb, disable_web_page_preview=True)
            except TelegramBadRequest:
                with suppress(TelegramBadRequest):
                    await tg_bot.edit_message_caption(chat_id=callback.message.chat.id, message_id=int(pid), caption=text_price, parse_mode="HTML", reply_markup=kb)
    except Exception:
        pass
    await state.set_state(PostFSM.caption)  # используем существующее состояние для текстового ввода
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            if post is not None:
                ...
                for k in ("autodelete_seconds", "autodelete_label", "autodelete_views", "autodelete_report", "autodelete_effective_seconds"):
                    if k in payload:
                        pl[k] = payload[k]
                    elif k in pl and k in {"autodelete_seconds", "autodelete_label", "autodelete_views"} and k not in payload:
                ...
                try:
                    ...
                    sec = int(pl.get("autodelete_effective_seconds") or pl.get("autodelete_seconds") or 0)
                    ...
                    if str(getattr(post, "status", "")) == "done" and sec > 0 and ids:
                        ...
                        owner = await session.get(Client, getattr(ch, "owner_id", 0)) if ch else None
                        report_on = bool(pl.get("autodelete_report", False))
