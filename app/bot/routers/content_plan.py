...
from app.bot.routers.shared_plan import build_calendar_kb as _build_calendar_kb, render_calendar as _render_calendar
...
async def _render_cp_channels_list(message_or_cb, user_id: int):
	...
	for ch in items:
		...
		try:
			...
			pass
		rows.append([InlineKeyboardButton(text=label, callback_data=f"cp_pick_channel_{ch.id}")])
...
async def _render_content_plan(callback: CallbackQuery, state: FSMContext, channel_id: int, center_date: datetime) -> None:
	...
	async with AsyncSessionLocal() as session:
		...
		if tz_code:
			try:
				...
				start_local = datetime(center_date.year, center_date.month, center_date.day, 0, 0, 0, tzinfo=loc)
				end_local = datetime(center_date.year, center_date.month, center_date.day, 23, 59, 59, 999999, tzinfo=loc)
				start = start_local.astimezone(timezone.utc)
				end = end_local.astimezone(timezone.utc)
			except Exception:
				# Фолбэк через фиксированный сдвиг формата UTC±HH[:MM]
				off = _offset_minutes_from_tz(tz_code)
				start = datetime(center_date.year, center_date.month, center_date.day, 0, 0, 0, tzinfo=timezone.utc) - timedelta(minutes=off)
				end = datetime(center_date.year, center_date.month, center_date.day, 23, 59, 59, 999999, tzinfo=timezone.utc) - timedelta(minutes=off)
		else:
			# Без TZ — считаем по UTC
			start = datetime(center_date.year, center_date.month, center_date.day, 0, 0, 0, tzinfo=timezone.utc)
			end = datetime(center_date.year, center_date.month, center_date.day, 23, 59, 59, 999999, tzinfo=timezone.utc)
	...
	try:
		...
		if uname:
			...
			with suppress(Exception):
				inv = await tg_bot.create_chat_invite_link(chat_id=int(ch.tg_chat_id), name="content-plan", creates_join_request=False)
				invite_url = getattr(inv, "invite_link", None)
			title_link = f"<a href=\"{html.escape(invite_url)}\">{html.escape(title_plain)}</a>" if invite_url else html.escape(title_plain)
	except Exception:
	...
	try:
		...
		async with AsyncSessionLocal() as session:
			...
			res = await session.execute(
				select(PostTask)
				.where((PostTask.channel_id == channel_id) & (PostTask.scheduled_at >= start) & (PostTask.scheduled_at <= end))
			...
			if not show_repeats and items:
				...
				for pp in items:
					...
					is_rep = (gid > 0) or (bool(plp.get("repeat_on")) and int(plp.get("repeat_seconds") or 0) > 0)
					if is_rep and gid > 0:
						prev = group_latest.get(gid)
						if (prev is None) or ((pp.scheduled_at or datetime.min) > (prev.scheduled_at or datetime.min)):
							group_latest[gid] = pp
					else:
						non_repeat.append(pp)
				items = sorted(non_repeat + list(group_latest.values()), key=lambda x: (x.scheduled_at or datetime.min))
			...
			for p in items:
				...
				if when:
					try:
						local_when = when.astimezone(ZoneInfo(tz_code)) if tz_code else when
					except Exception:
						local_when = when + timedelta(minutes=_offset_minutes_from_tz(tz_code))
					hm = local_when.strftime("%H:%M")
				else:
					hm = "--:--"
				pl = p.payload or {}
				if pl.get("type") == "text":
					first = (pl.get("text") or "").strip().splitlines()[0] or "Без названия"
				else:
				...
				row_btns = [InlineKeyboardButton(text=btn_text, callback_data=f"cp_open_post:{p.id}:{center_date.date().isoformat()}")]
				# Быстрая кнопка отключить автоповтор для серии
				try:
					if pl.get("repeat_on") and int(pl.get("repeat_seconds") or 0) > 0:
						rg = pl.get("repeat_group_id") or p.id
						row_btns.append(InlineKeyboardButton(text="⏹ Повтор выкл", callback_data=f"{CB.CP_REPEAT_OFF}:{rg}"))
				except Exception:
					pass
	...
	try:
		if len(post_rows) > max_rows:
			info_row = [InlineKeyboardButton(text=f"Показаны {max_rows} из {len(post_rows)}", callback_data="cp_day_center_nop")]
	...
	left = InlineKeyboardButton(text=_format_date_human(prev_day), callback_data=f"cp_day_prev:{channel_id}:{prev_day.date().isoformat()}")
	center_btn = InlineKeyboardButton(text=_format_date_human(center_date), callback_data="cp_day_center_nop")
	right = InlineKeyboardButton(text=_format_date_human(next_day), callback_data=f"cp_day_next:{channel_id}:{next_day.date().isoformat()}")
	open_cal = InlineKeyboardButton(text="Календарь", callback_data=f"cp_open_cal:{channel_id}:{center_date.date().isoformat()}")
	...
	btn_toggle = InlineKeyboardButton(text=btn_label, callback_data=CB.CP_TOGGLE_REPEATS)
...
async def cb_cp_repeat_off(callback: CallbackQuery, state: FSMContext):
	...
	from app.domain.models import PostTask
	async with AsyncSessionLocal() as session:
		res = await session.execute(select(PostTask).where(PostTask.status == "pending"))
...
@router.callback_query(F.data.startswith("cp_day_prev") | F.data.startswith("cp_day_next"))
...
async def cb_cp_edit_autodel(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        parts = (callback.message.reply_markup.inline_keyboard or [])
...
async def cb_cp_open_post(callback: CallbackQuery, state: FSMContext):
		...
		try:
			...
			if not link:
				...
				if mid:
					...
					link = f"https://t.me/{uname}/{mid}" if uname else f"https://t.me/c/{str(tg_chat_id)[4:]}/{mid}"
			if link:
	...
	try:
		async with AsyncSessionLocal() as session:
			...
			if post.scheduled_at is not None:
				try:
					local_dt = post.scheduled_at.astimezone(ZoneInfo(tz_code)) if tz_code else post.scheduled_at
				except Exception:
					local_dt = post.scheduled_at + timedelta(minutes=_offset_minutes_from_tz(tz_code))
				months = ["января","февраля","марта","апреля","мая","июня","июля","августа","сентября","октября","ноября","декабря"]
				wd = ["понедельник","вторник","среда","четверг","пятница","суббота","воскресенье"][local_dt.weekday()]
	...
	if status == "done":
		...
		try:
			...
			chan_title_cp = (ch.title if ch else None) or (str(tg_chat_id) if tg_chat_id else "канал")
			if uname:
				title_link_cp = f"[{_escape_markdown_label(chan_title_cp)}](https://t.me/{uname})"
			else:
				title_link_cp = _escape_markdown_label(chan_title_cp)
		except Exception:
			title_link_cp = _escape_markdown_label((ch.title if ch else None) or (str(tg_chat_id) if tg_chat_id else "канал"))
		...
		kb = InlineKeyboardMarkup(inline_keyboard=[
			[InlineKeyboardButton(text="Дублировать", callback_data=CB.EDIT_DUP), InlineKeyboardButton(text="Изменить", callback_data=f"cp_edit_post:{post.id}:{date_iso}")],
			[InlineKeyboardButton(text="☑️ Это рекламный пост", callback_data=CB.EDIT_AD_TOGGLE)],
			[InlineKeyboardButton(text=autodel_line, callback_data=CB.EDIT_AUTODEL)],
			[InlineKeyboardButton(text="Удалить", callback_data=f"cp_delete_post:{post.id}:{date_iso}")],
			[InlineKeyboardButton(text="← Назад", callback_data=f"cp_open_cal:{post.channel_id}:{date_iso}")],
		])
	else:
		# Добавим кликабельное имя канала
		try:
		...
		kb = InlineKeyboardMarkup(inline_keyboard=[
			...
			[InlineKeyboardButton(text="☑️ Это рекламный пост", callback_data=CB.EDIT_AD_TOGGLE)],
			[InlineKeyboardButton(text=autodel_line, callback_data=CB.EDIT_AUTODEL)],
			[InlineKeyboardButton(text="Изменить дату", callback_data=CB.POST_SETTINGS_DEFER), InlineKeyboardButton(text="🔥 Опубликовать", callback_data=CB.POST_SETTINGS_PUBLISH)],
			[InlineKeyboardButton(text="Удалить", callback_data=f"cp_delete_post:{post.id}:{date_iso}")],
			[InlineKeyboardButton(text="← Назад", callback_data=f"cp_open_cal:{post.channel_id}:{date_iso}")],
		])
...
async def cb_cp_edit_post(callback: CallbackQuery, state: FSMContext):
	...
	async with AsyncSessionLocal() as session:
		...
		pos = pl.get("media_pos", "top")  # top|bottom
		spoiler = bool(pl.get("media_spoiler", False))
		pl.pop("result_ids", None)
		pl.pop("result_link", None)
		pl.pop("notify_context", None)
	if mid is None:
		return await callback.answer("Не удалось определить сообщение для редактирования", show_alert=True)
	...
	try:
		...
		if old_prev_id:
			with suppress(TelegramBadRequest):
				await tg_bot.delete_message(chat_id=callback.message.chat.id, message_id=int(old_prev_id))
		with suppress(TelegramBadRequest):
...
def _build_calendar_kb_local(channel_id: int, focus: datetime, selected: datetime | None) -> InlineKeyboardMarkup:
    # совместимость; оставляем обёртку на общий хелпер
    return _build_calendar_kb(channel_id, focus, selected)

async def _render_calendar_local(callback: CallbackQuery, channel_id: int, focus_date: datetime, selected_date: datetime | None) -> None:
...
@router.callback_query(F.data.startswith("cp_month_prev:") | F.data.startswith("cp_month_next:"))
