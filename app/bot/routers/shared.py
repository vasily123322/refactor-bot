...
async def safe_answer(callback, text: str | None = None, show_alert: bool = False) -> None:
...
async def safe_edit_reply_markup(bot, chat_id: int, message_id: int, reply_markup) -> None:
    """Safely edit reply markup, ignoring TelegramBadRequest."""
    with suppress(TelegramBadRequest):
        await bot.edit_message_reply_markup(chat_id=chat_id, message_id=message_id, reply_markup=reply_markup)


async def apply_preview_media(bot, chat_id: int, prev_message_id: int | None, payload: dict) -> None:
    ...
    try:
        ...
        if media_type == "photo":
            ...
            with suppress(TelegramBadRequest):
                await bot.edit_message_media(chat_id=chat_id, message_id=int(prev_message_id), media=media)
...
async def back_restore_preview_from_caption(callback, state, send_preview_fn, tg_bot) -> bool:
...
async def back_restore_preview_from_content(callback, state, send_preview_fn, tg_bot) -> bool:
	...
	try:
		...
		if prompt_id:
			with suppress(TelegramBadRequest):
				await tg_bot.delete_message(chat_id=callback.message.chat.id, message_id=int(prompt_id))
		...
		is_edit_mode = (data_back.get("edit_chat_id") is not None and data_back.get("edit_msg_id") is not None and not bool(data_back.get("is_draft", False)))
		if is_edit_mode and (payload_back.get("type") in {"photo","video","animation","audio","voice","video_note","album"}):
			await state.update_data(preview_msg_id=preview2.message_id, media_prompt_id=None, ui_submenu="media")
			pl = payload_back
			pos = pl.get("media_pos", "top")
			spoiler = bool(pl.get("media_spoiler", False))
			label_pos = "Расположение: сверху" if pos != "bottom" else "Расположение: снизу"
			...
			media_kb = InlineKeyboardMarkup(inline_keyboard=[
				...
				[InlineKeyboardButton(text=label_sp, callback_data=CB.MEDIA_SPOILER_TOGGLE)],
				[InlineKeyboardButton(text="Заменить", callback_data=CB.MEDIA_REPLACE)],
				[InlineKeyboardButton(text="← Назад", callback_data=CB.POST_BACK)],
			])
			with suppress(TelegramBadRequest):
				await tg_bot.edit_message_reply_markup(chat_id=callback.message.chat.id, message_id=preview2.message_id, reply_markup=media_kb)
		else:
			await state.update_data(preview_msg_id=preview2.message_id, media_prompt_id=None, ui_submenu=None)
		await state.set_state(PostFSM.preview)
		with suppress(TelegramBadRequest):
			await callback.answer()
		return True
...
async def build_preview_kb(state_data: dict) -> "InlineKeyboardMarkup":
    ...
    if (payload.get("buttons") or []):
        ...
        return InlineKeyboardMarkup(inline_keyboard=user_rows + (base_kb.inline_keyboard or []))
    return base_kb
...
async def resolve_original_post_ref(message, bot) -> tuple[int | None, int | None]:
    ...
    try:
        ...
        if origin and getattr(origin, "chat", None) and getattr(origin, "message_id", None):
            try:
        ...
        if (orig_chat_id is None or orig_msg_id is None) and getattr(message, "forward_from_chat", None) and getattr(message, "forward_from_message_id", None):
            try:
        ...
        # 3) t.me link
        if (orig_chat_id is None or orig_msg_id is None) and (message.text or "").startswith("http"):
            import re as _re
            txt = message.text or ""
            m = _re.search(r"t\.me/(?:c/([0-9]+)/([0-9]+)|([A-Za-z0-9_]+)/([0-9]+))", txt)
            if m:
...
def build_payload_from_message(message, *, prefer_html_caption: bool = False, include_entities: bool = False) -> dict | None:
    ...
    try:
        ...
        if message.photo:
            ph = message.photo[-1]
            payload = {"type": "photo", "file_id": ph.file_id, "caption": _pick_caption()}
            ...
            payload = {"type": "video", "file_id": message.video.file_id, "caption": _pick_caption()}
            if include_entities and getattr(message, "caption_entities", None):
                payload["caption_entities"] = [e.model_dump() for e in (message.caption_entities or [])]
        elif message.animation:
            payload = {"type": "animation", "file_id": message.animation.file_id, "caption": _pick_caption()}
            if include_entities and getattr(message, "caption_entities", None):
                payload["caption_entities"] = [e.model_dump() for e in (message.caption_entities or [])]
        elif message.audio:
            payload = {"type": "audio", "file_id": message.audio.file_id, "caption": _pick_caption()}
            if include_entities and getattr(message, "caption_entities", None):
                payload["caption_entities"] = [e.model_dump() for e in (message.caption_entities or [])]
        elif message.voice:
            payload = {"type": "voice", "file_id": message.voice.file_id, "caption": _pick_caption()}
            if include_entities and getattr(message, "caption_entities", None):
                payload["caption_entities"] = [e.model_dump() for e in (message.caption_entities or [])]
        elif message.video_note:
            payload = {"type": "video_note", "file_id": message.video_note.file_id, "caption": _pick_caption()}
            if include_entities and getattr(message, "caption_entities", None):
                payload["caption_entities"] = [e.model_dump() for e in (message.caption_entities or [])]
...
def is_admin_user(user_id: int | None) -> bool:
    ...
    try:
        ...
        return bool(_settings.admin_user_id) and (int(user_id or 0) == int(_settings.admin_user_id))
    except Exception:
        return False
