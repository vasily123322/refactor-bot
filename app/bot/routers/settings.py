...
async def cb_post_replace_autosign(callback: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not autosign_text:
            return await callback.answer(
                "В настройках канала пустая автоподпись", show_alert=True
            )
        ...
        def _extract_entities(md: str) -> tuple[str, list[dict]]:
            ...
            for start, end, ent, repl in parts:
                ...
                offset_in_clean = (
                    sum(len(s.encode("utf-16-le")) for s in clean_chunks) // 2
                )
        ...
        def _apply_footer(
            base: str, base_entities: list[dict] | None, footer_md: str, limit: int
        ...
        def _strip_old_suffix(
            s: str, entities: list[dict] | None, limit_for_info: int, old: str
        ) -> tuple[str, list[dict] | None]:
            if not old:
                return (s, entities)
            candidates = [SEP + old, "\n" + old, "\n\n" + old, old]
        ...
        rows = await session.execute(
            select(PostTask).where(
                PostTask.channel_id == chan_id, PostTask.status == "done"
            )
        )
    ...
    for post in posts:
        ...
        try:
            if type_ == "text":
                ...
                text1, ents1 = _strip_old_suffix(
                    text0, ents0, 4096, OLD_AUTOSIGN.strip()
                )
                final, ents_final = _apply_footer(text1, ents1, autosign_text, 4096)
                with suppress(TelegramBadRequest):
                    await tg_bot.edit_message_text(
                        chat_id=tg_chat_id,
                        message_id=int(msg_ids[-1]),
                        text=final,
                        entities=ents_final,
                        disable_web_page_preview=True,
                    )
    ...
    await callback.answer(
        f"Готово: ок={ok_count}, ошибок={fail_count}", show_alert=True
    )
