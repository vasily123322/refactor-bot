...
async def run_bot() -> None:
	...
	try:
		...
		logger.info(f"DB: engine initialized, pool={type(pool_name).__name__ if pool_name else 'unknown'} staticpool={getattr(settings,'sqla_staticpool',False)} nullpool={getattr(settings,'sqla_nullpool',False)}")
	except Exception:
		pass
	...
	try:
		...
		logger.warning(f"Userbot не запущен: {e}. Клонирование временно отключено. Выполните авторизацию userbot.")

	try:
		logger.info("Boot: creating services...")
		if Scheduler is None and '_scheduler_import_err' in globals() and _scheduler_import_err is not None:
			logger.warning(f"Scheduler не загружен: {_scheduler_import_err}")
		if GrabPoller is None and '_grab_import_err' in globals() and _grab_import_err is not None:
