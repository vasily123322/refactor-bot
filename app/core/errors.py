        ...
        try:
            ...
            try:
                ...
                if hasattr(event, "message") and event.message:
                    user_id = getattr(
                        getattr(event.message, "from_user", None), "id", None
                    )
                    chat_id = getattr(getattr(event.message, "chat", None), "id", None)
                elif hasattr(event, "callback_query") and event.callback_query:
                    user_id = getattr(
                        getattr(event.callback_query, "from_user", None), "id", None
                    )
                    chat_id = getattr(
                        getattr(
                            getattr(event.callback_query, "message", None), "chat", None
                        ),
                        "id",
                        None,
                    )
                logger.bind(user_id=user_id, chat_id=chat_id).exception(
                    f"Unhandled error: {e}"
                )
            except Exception:
