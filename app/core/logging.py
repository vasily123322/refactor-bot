		...
		for a in args:
			try:
				...
				if m and chat_id is None:
					chat_id = int(getattr(m, "chat", getattr(a, "chat", None)).id if hasattr(m, "chat") else getattr(m, "id", 0))
				if hasattr(a, "data") and data is None:
					data = getattr(a, "data")
			except Exception:
				continue
		logger.bind(user_id=user_id, chat_id=chat_id).info(f"handler={handler.__name__} data={data}")
