...
class GrabPoller:
	def __init__(self, interval_seconds: int = 30):
		...
		self._loop = PollingLoop(interval_seconds=interval_seconds, on_tick=self._tick, name="grab_poller", jitter_seconds=0)
		...
		try:
			...
			for s in sources:
				...
				if last_id is not None and m.id <= last_id:
					continue
				# Извлечём текст
				orig_text = (getattr(m, "caption", None) or getattr(m, "text", None) or "").strip()
				if not orig_text:
					self._last_processed[int(s.id)] = int(m.id)
					continue
					...
					if mode_raw == "custom":
						force_custom = True
						mode = "rewrite"
					instruction = {"summary": "создай краткое саммари текста для поста в Telegram",
							   "rewrite": "перепиши текст своими словами, сохранив смысл",
							   "paraphrase": "перефразируй текст, сделай его более понятным"}.get(mode, "создай краткое саммари текста для поста в Telegram")
					...
					if not result.get("success"):
						logger.warning(f"GrabPoller: generation failed for channel_id={s.channel_id}: {result.get('error')}")
						self._last_processed[int(s.id)] = int(m.id)
						continue
					text_out = result.get("text") or ""
					# Цитирование источника
					text_out = await self._append_citation(text_out, join_target, m.id, s.citation_enabled)
					...
					try:
						...
						if target_chat_id > 0:
							logger.warning(f"GrabPoller: skip send, suspicious tg_chat_id={target_chat_id} for channel_id={s.channel_id}")
							self._last_processed[int(s.id)] = int(m.id)
							continue
						posting = PostingService(tg_bot, AsyncSessionLocal)
						ids = await posting.send_now(channel_id=target_chat_id, payload={"type": "text", "text": text_out, "silent": True})
						if ids:
							logger.info(f"GrabPoller: posted to target_tg={target_chat_id} msg_ids={ids}")
						self._last_processed[int(s.id)] = int(m.id)
					except Exception as e:
						logger.exception(f"GrabPoller: history/process error for source_id={s.id}: {e}")
				# end for each source
		except Exception as e:
	...
	async def _append_citation(self, text: str, join_target: str, msg_id: int, enabled: bool) -> str:
		if not enabled:
			return text
		try:
