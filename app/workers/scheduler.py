...
from app.services.scheduling import as_utc as _as_utc_svc, compute_next_repeat_time as _compute_next_repeat_time_svc, cleanup_runtime_fields as _cleanup_rt_svc, inherit_flags_for_repeat as _inherit_flags_svc

SCHEDULER_BUILD_ID = "2025-10-22T16:05-autodel-precheck-fallback"


class Scheduler:
	def __init__(self, session_or_factory, posting: PostingService, interval_seconds: int = 5):
		self.session = session_or_factory if isinstance(session_or_factory, AsyncSession) else None
		self.session_factory = session_or_factory if not isinstance(session_or_factory, AsyncSession) else None
	...
	@staticmethod
	def _compute_next_repeat_time(start_utc: datetime, repeat_seconds: int, after_utc: datetime) -> datetime:
	...
	async def _del_later(self, bot, chat_id: int, msg_ids: list[int], delay: int, post_id_val: int, report: bool, link_val: str | None):
		...
		try:
			...
			for mid in msg_ids:
				try:
					...
					pass
			if success_count > 0:
				logger.info(f"Scheduler: autodelete done for post id={post_id_val} messages={success_count}/{len(msg_ids)}")
			else:
				logger.warning(f"Scheduler: autodelete failed (no messages deleted) for post id={post_id_val}")
			...
			try:
				async with AsyncSessionLocal() as s2:
					...
					if p2:
						...
						if success_count > 0 and ((int(pl2.get("autodelete_effective_seconds") or pl2.get("autodelete_seconds") or 0) > 0) or (int(pl2.get("autodelete_views") or 0) > 0)):
							...
							await s2.commit()
							if report:
								owner2 = await s2.get(Client, getattr(await s2.get(Channel, int(p2.channel_id)), "owner_id", 0))
								if owner2 and getattr(owner2, "tg_user_id", None):
									with suppress(Exception):
										await bot.send_message(chat_id=int(owner2.tg_user_id), text=("🗑️ Пост удалён по таймеру\n" + (pl2.get("result_link") or "")), disable_web_page_preview=True)
			except Exception:
				pass
		except Exception:
	...
	async def _apply_autodelete(self, session: AsyncSession, post: PostTask, tg_chat_id: int, pl: dict, ids: list[int]) -> None:
		...
		try:
			...
			try:
				if bool(pl.get("repeat_on", False)) and rep_sec > 0 and use_sec == rep_sec:
			...
			if use_sec > 0 and ids:
				...
				pl["autodelete_seconds"] = int(max(int(pl.get("autodelete_seconds") or 0), use_sec))
				if align_to_repeat and getattr(post, "scheduled_at", None) is not None:
					base = post.scheduled_at
					base_aware = base if (getattr(base, "tzinfo", None) is not None) else base.replace(tzinfo=_tz.utc)
				...
				try:
					logger.info(f"Scheduler: set autodelete post_id={int(post.id)} scheduled_at={(getattr(post,'scheduled_at',None).isoformat() if getattr(post,'scheduled_at',None) else 'None')} autodelete_at={pl['autodelete_at']}")
				except Exception:
					pass
				# локальный таймер
				try:
					delay = max(0, int((due - _dt.now(_tz.utc)).total_seconds()))
					asyncio.create_task(self._del_later(self.posting.bot, tg_chat_id, list(ids), delay, int(post.id), bool(pl.get("autodelete_report", False)), pl.get("result_link")))
				except Exception:
					pass
			else:
				try:
			...
			logger.exception(f"Scheduler: autodelete schedule error for post id={int(post.id)}: {e}")
	...
	async def start(self) -> None:
		...
		if self._del_task is None or self._del_task.done():
			self._del_task = asyncio.create_task(self._run_deletor(), name="scheduler-autodelete")
	...
	async def _skip_overdue_repeat_and_schedule_next(self, session: AsyncSession, post: PostTask, pl: dict) -> bool:
		"""Вернёт True, если пост был пропущен из-за просрочки и запланирован следующий повтор."""
		try:
			...
			if sched is None:
				return False
			sched_aware = sched if (getattr(sched, "tzinfo", None) is not None) else sched.replace(tzinfo=timezone.utc)
			sched_aware = sched_aware.astimezone(timezone.utc)
			if sched_aware > boot_time:
				return False
			post.status = "skipped"
			await session.commit()
			try:
				secs_rep = int(pl.get("repeat_seconds") or 0)
				if secs_rep > 0:
					next_when = sched_aware
					while next_when <= boot_time:
						next_when = next_when + timedelta(seconds=secs_rep)
					pl_next = dict(pl)
					pl_next.pop("result_ids", None)
					pl_next.pop("result_link", None)
					pl_next.pop("autodeleted", None)
					pl_next.pop("autodeleted_at", None)
					if pl_next.get("autosign_applied"):
						pl_next["autosign_applied"] = True
					if pl_next.get("repeat_group_id") is None:
						pl_next["repeat_group_id"] = int(post.id)
					new_post = PostTask(channel_id=int(post.channel_id), payload=pl_next, dedupe_key=None, scheduled_at=next_when)
	...
	async def _dedupe_repeat_series(self, session: AsyncSession, post: PostTask, pl: dict) -> None:
		try:
			if not bool(pl.get("repeat_on", False)):
				return
			...
			others = await session.execute(
				select(PostTask)
				.where((PostTask.status == "pending") & (PostTask.channel_id == int(post.channel_id)))
			)
			for p2 in list(others.scalars().all()):
				if p2.id == post.id:
					continue
				pl2 = dict(p2.payload or {})
				if pl2.get("repeat_group_id") == rg and p2.scheduled_at and p2.scheduled_at <= datetime.now(timezone.utc):
					p2.status = "skipped"
					await session.commit()
		except Exception:
			pass

	async def _boot_cleanup_repeats(self, session: AsyncSession, items: list[PostTask]) -> list[PostTask]:
		try:
			if (not self._boot_cleanup_done) and (self._boot_time is not None) and items:
				boot_time = self._boot_time
				group_seen: dict[int, bool] = {}
				overdue: list[PostTask] = []
				for p in list(items):
					pl0 = dict(getattr(p, "payload", None) or {})
					if not bool(pl0.get("repeat_on", False)):
						continue
					when = getattr(p, "scheduled_at", None)
					if when is None:
						continue
					when_aware = self._as_utc(when)
					if when_aware <= boot_time:
						overdue.append(p)
				if overdue:
					for p in overdue:
						p.status = "skipped"
					...
					for p in overdue:
						pl0 = dict(getattr(p, "payload", None) or {})
						secs_rep = int(pl0.get("repeat_seconds") or 0)
						if secs_rep <= 0:
							continue
						rg = pl0.get("repeat_group_id")
						if rg is None:
							rg = int(p.id)
						if group_seen.get(int(rg)):
							continue
						group_seen[int(rg)] = True
						when = getattr(p, "scheduled_at", None)
						when_aware = self._as_utc(when)
						next_when = self._compute_next_repeat_time(when_aware, secs_rep, boot_time)
						pl_next = self._cleanup_runtime_fields(pl0)
						try:
							ad_base = int(pl0.get("autodelete_seconds") or pl0.get("autodelete_effective_seconds") or 0)
							if ad_base > 0:
								pl_next["autodelete_seconds"] = ad_base
							pl_next.pop("autodelete_at", None)
						except Exception:
							pass
						pl_next = self._inherit_flags_for_repeat(pl_next, int(p.id))
						if int(rg) not in self._boot_group_scheduled:
							new_post = PostTask(channel_id=int(p.channel_id), payload=pl_next, dedupe_key=None, scheduled_at=next_when)
							session.add(new_post)
							await session.commit()
							self._boot_group_scheduled.add(int(rg))
					# Уберём их из текущей партии
					items = [p for p in items if p not in overdue]
			self._boot_cleanup_done = True
		except Exception:
			pass
		return items

	async def _prevent_repeat_overflow(self, session: AsyncSession, items: list[PostTask]) -> None:
		try:
			if items:
				...
				for rg, lst in groups.items():
					...
					if len(lst) > limit:
						lst_sorted = sorted([x for x in lst if x.scheduled_at is not None], key=lambda x: x.scheduled_at)
						keep = None
						for x in lst_sorted:
							if x.scheduled_at and x.scheduled_at > datetime.now(timezone.utc):
								keep = x
								break
		...
		try:
			if items:
				...
				await session.execute(
					update(PostTask).where(PostTask.id.in_(ids_to_mark)).values(status="processing")
				)
				await session.commit()
		except Exception:
			pass

	async def _send_admin_log_if_configured(self, tg_chat_id: int, ids: list[int], pl: dict) -> None:
		try:
			...
			async with _Sess() as _slog:
				...
				try:
					...
					if uname:
						...
						inv = await self.posting.bot.create_chat_invite_link(chat_id=tg_chat_id, name="post-log", creates_join_request=False)
				...
				with _s(Exception):
					await self.posting.bot.send_message(log_chat_id, text_log, disable_web_page_preview=True)
		except Exception:
			pass

	async def _persist_result_fields(self, session: AsyncSession, post: PostTask, tg_chat_id: int, pl: dict, ids: list[int]) -> None:
		try:
	...
	async def _pin_if_needed(self, tg_chat_id: int, ids: list[int], pl: dict) -> None:
		try:
			if bool(pl.get("pin_on", False)) and ids:
				with suppress(Exception):
					await self.posting.bot.pin_chat_message(chat_id=tg_chat_id, message_id=int(list(ids)[-1]))
		except Exception:
			pass

	async def _forward_if_needed(self, session: AsyncSession, tg_chat_id: int, ids: list[int], pl: dict) -> None:
		try:
			fw: list[int] = []
			try:
			...
			if fw and ids:
				for t_cid in fw:
					try:
						...
						for mid in list(ids):
							with suppress(Exception):
								await self.posting.bot.forward_message(chat_id=target_chat_id, from_chat_id=tg_chat_id, message_id=int(mid), disable_notification=bool(pl.get("silent", False)))
					except Exception:
						continue
		except Exception:
			pass

	async def _notify_owner_published(self, session: AsyncSession, ch: Channel, post: PostTask, tg_chat_id: int, ids: list[int], pl: dict) -> None:
		try:
			...
			chan_line = f"Канал: <a href=\"{chan_link}\">{title_esc}</a> | Автор: {author}" if locals().get("chan_link") else f"Канал: {title_esc} | Автор: {author}"
			...
			if owner:
				...
				pl_note["notify_context"] = {"type": "published_notice", "post_id": post.id, "date": local_dt.date().isoformat()}
				...
				await self.posting.bot.send_message(chat_id=int(owner.tg_user_id), text=text, disable_web_page_preview=True, reply_markup=kb, parse_mode="HTML")
		except Exception:
			pass

	async def _schedule_next_repeat_if_needed(self, session: AsyncSession, post: PostTask, pl: dict) -> None:
		try:
			if not bool(pl.get("repeat_on", False)):
				return
			secs_rep = int(pl.get("repeat_seconds") or 0)
			if secs_rep <= 0:
				return
			base_dt = getattr(post, "scheduled_at", None)
			if base_dt is not None:
				base_aware = base_dt if (getattr(base_dt, "tzinfo", None) is not None) else base_dt.replace(tzinfo=timezone.utc)
				base_aware = base_aware.astimezone(timezone.utc)
			else:
				base_aware = datetime.now(timezone.utc)
			next_when = base_aware + timedelta(seconds=secs_rep)
			while next_when <= datetime.now(timezone.utc):
				next_when = next_when + timedelta(seconds=secs_rep)
			pl_next = dict(pl)
			pl_next.pop("result_ids", None)
			pl_next.pop("result_link", None)
			...
			pl_next.pop("autodeleted_at", None)
			try:
				ad_base = int(pl.get("autodelete_seconds") or pl.get("autodelete_effective_seconds") or 0)
				ad_eff_prev = int(pl.get("autodelete_effective_seconds") or 0)
				if ad_base > 0:
					pl_next["autodelete_seconds"] = ad_base
				logger.debug(f"Scheduler: repeat inherit autodelete base={ad_base} eff_prev={ad_eff_prev} for parent id={int(post.id)}")
			except Exception:
				pass
			try:
				ad_base2 = int(pl_next.get("autodelete_seconds") or pl.get("autodelete_effective_seconds") or pl.get("autodelete_seconds") or 0)
				if ad_base2 > 0:
					from datetime import datetime as _dt, timezone as _tz, timedelta as _td
					pl_next["autodelete_at"] = (next_when + _td(seconds=int(ad_base2))).isoformat()
					logger.debug(f"Scheduler: repeat set autodelete_at={pl_next['autodelete_at']} for next post")
			except Exception:
				pass
			if pl_next.get("autosign_applied"):
				pl_next["autosign_applied"] = True
			...
			session.add(new_post)
			await session.commit()
			logger.info(f"Scheduler: repeat scheduled next post for id={post.id} at={next_when}")
		except Exception:
			pass
				...
				if self.session_factory is not None:
					async with self.session_factory() as session:
						...
						res = await session.execute(
							select(PostTask)
							.where((PostTask.status == "pending") & (PostTask.scheduled_at != None) & (PostTask.scheduled_at <= now))
							.order_by(PostTask.scheduled_at.asc())
							.limit(10)
						)
	...
	async def _process_items(self, session: AsyncSession, items: list[PostTask]) -> None:
		if not items:
			return
		for post in items:
			try:
				logger.info(f"Scheduler: dispatch post id={post.id} channel={post.channel_id} at={post.scheduled_at}")
				...
				ids = await self.posting.send_now(tg_chat_id, dict(pl, _via_scheduler=True, _post_task_id=int(post.id), _also_schedule_autodelete=True))
				# Отправим лог админу о посте
				await self._send_admin_log_if_configured(tg_chat_id, ids, pl)
				if ids:
					await self._persist_result_fields(session, post, tg_chat_id, pl, ids)
					# Планирование автоудаления сразу после успешной отправки
					try:
						use0 = int(pl.get("autodelete_effective_seconds") or pl.get("autodelete_seconds") or 0)
						logger.info(f"Scheduler: autodel pre-check post_id={int(post.id)} use_sec={use0} ids={len(ids) if ids else 0}")
					except Exception:
						pass
					await self._apply_autodelete(session, post, tg_chat_id, pl, list(ids))
					...
					try:
						...
						if sec_fb > 0 and (len(ids) > 0) and not pl.get("autodelete_at"):
							from datetime import datetime as _dt, timezone as _tz, timedelta as _td
							due_fb = _dt.now(_tz.utc) + _td(seconds=sec_fb)
							pl["autodelete_at"] = due_fb.isoformat()
							post.payload = pl
							await session.commit()
							logger.info(f"Scheduler: autodelete fallback set post_id={int(post.id)} autodelete_at={pl['autodelete_at']}")
					except Exception:
						pass
					...
					await self._notify_owner_published(session, ch, post, tg_chat_id, ids, pl)
	...
	async def _process_due_deletions(self, session: AsyncSession) -> None:
		...
		res = await session.execute(
			...
			.where(
				...
				(
					(PostTask.payload['autodelete_at'].as_string() != None) |
					(PostTask.payload['autodelete_seconds'].as_integer() > 0)
				) &
				(
					(PostTask.payload['autodeleted'].as_boolean() == None) |
					(PostTask.payload['autodeleted'].as_boolean() == False)
				)
			)
		...
		for post in items:
			...
			try:
				sec_dbg = int(pl.get("autodelete_effective_seconds") or pl.get("autodelete_seconds") or 0)
			except Exception:
				sec_dbg = 0
			has_ad_at = bool(pl.get("autodelete_at"))
			logger.info(f"Scheduler: due candidate id={int(post.id)} has_autodelete_at={has_ad_at} sec={sec_dbg} ids={len(ids)}")
			...
			logger.info(f"Scheduler: due deletion check id={int(post.id)} due_at={due_at.isoformat()} now={now.isoformat()} ids={len(ids)}")
			if due_at > now:
				continue
			ch = await session.get(Channel, post.channel_id)
			if not ch:
			...
			for mid in ids:
				try:
					await self.posting.bot.delete_message(chat_id=chat_id, message_id=int(mid))
					success += 1
				except Exception as e:
					logger.warning(f"Scheduler: delete failed chat_id={chat_id} msg_id={int(mid)} err={e}")
					msg = str(e).lower()
					if ("message to delete not found" in msg) or ("message_id_invalid" in msg):
						not_found += 1
					elif ("can't be deleted" in msg) or ("message can't be deleted" in msg):
						cannot_delete += 1
			...
			if (success > 0) or (not_found == len(ids)) or (cannot_delete == len(ids)):
				...
				await session.commit()
				if success > 0:
					logger.info(f"Scheduler: autodeleted success id={int(post.id)} deleted={success}/{len(ids)}")
			else:
				logger.warning(f"Scheduler: autodelete failed (no messages deleted) for post id={int(post.id)}")
				...
				if pl.get("autodelete_report"):
					...
					if owner and getattr(owner, "tg_user_id", None):
						...
						try:
							# Заголовок поста
							if (pl.get("type") == "text"):
								title_line = (pl.get("text") or "").strip().splitlines()[0]
							else:
							...
							try:
								...
								tz_code2 = (st2.filters or {}).get("tz") if (st2 and st2.filters) else None
								sched = getattr(post, "scheduled_at", None)
								if sched is not None:
									sched_aware = sched if (getattr(sched, "tzinfo", None) is not None) else sched.replace(tzinfo=timezone.utc)
									local_dt = sched_aware
									with suppress(Exception):
										local_dt = sched_aware.astimezone(ZoneInfo(tz_code2)) if tz_code2 else sched_aware
									local_str = f"{local_dt.day} {months[local_dt.month-1]} {local_dt.year} {local_dt.strftime('%H:%M')} ({weekdays[local_dt.weekday()]})"
							except Exception:
								pass
							chan_name = getattr(ch, "title", None) or str(getattr(ch, "tg_chat_id", ""))
							# Параметры удаления
							vv = int(pl.get("autodelete_views") or 0)
							views_line2 = f"👁 Просмотры: {vv}" if vv > 0 else ""
							lab_views = (f"{vv//1000}к" if (vv >= 1000 and vv % 1000 == 0) else str(vv)) if vv > 0 else None
							...
							with suppress(Exception):
								await self.posting.bot.send_message(chat_id=int(owner.tg_user_id), text=text2, disable_web_page_preview=True)
						except Exception:
							...
							if link:
								text = f"{text}\n{link}"
							with suppress(Exception):
								await self.posting.bot.send_message(chat_id=int(owner.tg_user_id), text=text, disable_web_page_preview=True)
