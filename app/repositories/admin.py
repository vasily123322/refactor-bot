...
class AdminConfigRepo:
    ...
    async def get(self) -> AdminConfig | None:
        res = await self.session.execute(
            select(AdminConfig).order_by(AdminConfig.id.asc()).limit(1)
        )
...
class BansRepo:
    ...
    async def is_banned(self, tg_chat_id: int) -> bool:
        res = await self.session.execute(
            select(BannedChat).where(BannedChat.tg_chat_id == tg_chat_id)
        )
        return res.scalars().first() is not None

    async def ban(
        self, tg_chat_id: int, reason: str | None, created_by: int | None
    ) -> BannedChat:
        res = await self.session.execute(
            select(BannedChat).where(BannedChat.tg_chat_id == tg_chat_id)
        )
    ...
    async def unban(self, tg_chat_id: int) -> bool:
        await self.session.execute(
            delete(BannedChat).where(BannedChat.tg_chat_id == tg_chat_id)
        )
        await self.session.commit()
        return True
    ...
    async def list_bans(self, limit: int = 50) -> list[BannedChat]:
        res = await self.session.execute(
            select(BannedChat).order_by(BannedChat.created_at.desc()).limit(limit)
        )
