...
class JoinRequestsRepo:
	...
	async def list_pending(self, channel_id: int, limit: int = 20, offset: int = 0) -> list[JoinRequest]:
		res = await self.session.execute(
			select(JoinRequest)
			.where(JoinRequest.channel_id == channel_id, JoinRequest.status == "pending")
	...
	async def get(self, channel_id: int, user_id: int) -> JoinRequest | None:
		res = await self.session.execute(
			select(JoinRequest).where(JoinRequest.channel_id == channel_id, JoinRequest.user_id == user_id)
		)
		return res.scalars().first()

	async def delete(self, channel_id: int, user_id: int) -> None:
		await self.session.execute(
			delete(JoinRequest).where(JoinRequest.channel_id == channel_id, JoinRequest.user_id == user_id)
		)
		await self.session.commit()

	async def list_pending_solved(self, channel_id: int, limit: int) -> list[JoinRequest]:
	...
	async def list_pending_filtered(
		...
		if solved_only:
			conds.append(or_(JoinRequest.challenge_type.is_(None), JoinRequest.attempts_left == 0))
		if invite_like:
			like = f"%{invite_like}%"
			conds.append(JoinRequest.invite_link.like(like))
		if minutes and minutes > 0:
			conds.append(JoinRequest.created_at > (datetime.now(timezone.utc) - timedelta(minutes=int(minutes))))
		res = await self.session.execute(
			select(JoinRequest)
			.where(and_(*conds))
			.order_by(JoinRequest.created_at.asc())
