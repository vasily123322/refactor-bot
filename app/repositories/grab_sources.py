...
class GrabSourcesRepo:
	...
	async def list_for_target(self, channel_id: int) -> list[GrabSource]:
		res = await self.session.execute(select(GrabSource).where(GrabSource.target_channel_id == channel_id))
		return list(res.scalars().all())

	async def add(self, source_chat_id: int, target_channel_id: int) -> GrabSource:
		obj = GrabSource(source_chat_id=source_chat_id, target_channel_id=target_channel_id)
