...
class AIPresetsRepo:
	...
	async def get_by_id(self, preset_id: int) -> AIPreset | None:
		res = await self.session.execute(select(AIPreset).where(AIPreset.id == preset_id))
	...
	async def list_active(self) -> list[AIPreset]:
		res = await self.session.execute(
			select(AIPreset).where(AIPreset.is_active == True).order_by(AIPreset.id)
		)
		return list(res.scalars().all())
	
	async def create(self, code: str, title: str, system_prompt: str, user_template: str, 
	                 description: str | None = None, rules: dict | None = None, 
	                 defaults: dict | None = None) -> AIPreset:
		preset = AIPreset(
			code=code,
			title=title,
...
class ChannelAISettingsRepo:
	...
	async def update_custom_prompt(self, channel_id: int, system_prompt: str | None, user_template: str | None) -> bool:
		settings = await self.get_or_create(channel_id)
...
class AISourcesRepo:
	...
	async def list_by_channel(self, channel_id: int) -> list[AISource]:
		res = await self.session.execute(
			select(AISource).where(AISource.channel_id == channel_id).order_by(AISource.id)
		)
		return list(res.scalars().all())
	
	async def get_by_id(self, source_id: int) -> AISource | None:
		res = await self.session.execute(select(AISource).where(AISource.id == source_id))
		return res.scalars().first()
	
	async def create(self, channel_id: int, source_type: str, source_value: str, 
	                 mode: str = "summary", citation_enabled: bool = True) -> AISource:
		source = AISource(
			channel_id=channel_id,
	...
	async def list_telegram_matches(self, chat_id: int | None = None, username: str | None = None) -> list[AISource]:
		...
		"""
		base = [AISource.enabled == True, AISource.source_type == "telegram"]
		...
		if chat_id and username:
			match = or_(
				...
				func.lower(AISource.source_value) == func.lower(func.concat("https://t.me/", cast(username, String)))
			)
		elif username:
			match = or_(
				func.lower(AISource.source_value) == func.lower(username),
	...
	async def update_value(self, source_id: int, source_type: str | None = None, source_value: str | None = None) -> bool:
		source = await self.get_by_id(source_id)
		if source:
	...
	async def list_all_telegram_enabled(self) -> list[AISource]:
		res = await self.session.execute(
			select(AISource).where(AISource.source_type == "telegram", AISource.enabled == True)
		)
		return list(res.scalars().all())
