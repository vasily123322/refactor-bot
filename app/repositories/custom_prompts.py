	...
	async def get_active(self, channel_id: int) -> AICustomSystemPrompt | None:
		res = await self.session.execute(
			...
			.where(
				...
				AICustomSystemPrompt.is_active == True,
			)
		)
		return res.scalars().first()

	async def create(self, channel_id: int, content: str, *, active: bool = False) -> AICustomSystemPrompt:
		obj = AICustomSystemPrompt(channel_id=channel_id, content=content, is_active=bool(active))
