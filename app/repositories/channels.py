...
from app.domain.models import Channel, ChannelSettings
...
class ChannelsRepo(BaseRepository[Channel]):
    ...
    async def create(
        self, owner_id: int, tg_chat_id: int, title: str | None
    ) -> Channel:
        channel = Channel(owner_id=owner_id, tg_chat_id=tg_chat_id, title=title)
        self.session.add(channel)
        await self.session.flush()
        self.session.add(
            ChannelSettings(channel_id=channel.id, autosign=None, split_rules=[])
        )
    ...
    async def assign_owner_and_title(
        self, channel_id: int, owner_id: int, title: str | None
    ) -> bool:
        obj = await self.session.get(Channel, channel_id)
        if not obj:
            return False
