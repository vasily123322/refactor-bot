...
from sqlalchemy import BigInteger, Integer, String, Boolean, ForeignKey, DateTime, Text, JSON, func, UniqueConstraint, Index
from app.core.db import Base
from app.domain.mixins import TimestampHelpersMixin, OwnerHelpersMixin, ActivatableHelpersMixin, UuidHelpersMixin
...
class Channel(CreatedAtMixin, OwnerHelpersMixin, ActivatableHelpersMixin, TimestampHelpersMixin, Base):
	...
	grab_sources: Mapped[list[GrabSource]] = relationship(back_populates="target_channel", cascade="all, delete-orphan")
...
class ChannelSettings(TimestampHelpersMixin, Base):
	...
	filters: Mapped[dict] = mapped_column(JSON, default={"url": 1, "audio": 1, "video": 1, "photo": 1, "text": 1, "animation": 1})
...
class GrabSource(TimestampHelpersMixin, Base):
	...
	__table_args__ = (
		UniqueConstraint("source_chat_id", "target_channel_id", name="uq_grab_source_target"),
	)

	id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
	source_chat_id: Mapped[int] = mapped_column(BigInteger, index=True)
	target_channel_id: Mapped[int] = mapped_column(ForeignKey("channels.id", ondelete="CASCADE"))
	filter_flags: Mapped[dict] = mapped_column(JSON, default={"url": 0, "audio": 1, "video": 1, "photo": 1, "text": 1, "animation": 1})
...
class PostTask(CreatedAtMixin, TimestampHelpersMixin, Base):
	...
	__table_args__ = (
		Index("ix_post_dedupe", "dedupe_key", unique=True),
	)
...
class ChannelAISettings(CreatedAtMixin, TimestampHelpersMixin, Base):
	...
	preset_id: Mapped[int | None] = mapped_column(ForeignKey("ai_presets.id", ondelete="SET NULL"))
...
class AIConversation(TimestampHelpersMixin, Base):
	...
	channel_id: Mapped[int | None] = mapped_column(ForeignKey("channels.id", ondelete="CASCADE"))
...
class AICustomSystemPrompt(CreatedAtMixin, TimestampHelpersMixin, Base):
	...
	__table_args__ = (
		Index("ix_csp_channel_active", "channel_id", "is_active"),
	)
...
class ExternalBot(CreatedAtMixin, TimestampHelpersMixin, Base):
	...
	__table_args__ = (
		UniqueConstraint("bot_user_id", name="uq_extbot_user"),
	)
	...
	owner_client_id: Mapped[int | None] = mapped_column(ForeignKey("clients.id", ondelete="SET NULL"))
	is_active: Mapped[bool] = mapped_column(Boolean, default=True)
...
class ModLog(CreatedAtMixin, TimestampHelpersMixin, Base):
	...
	id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
	channel_id: Mapped[int] = mapped_column(ForeignKey("channels.id", ondelete="CASCADE"))
	action: Mapped[str] = mapped_column(String(32))  # approve|reject|mode_change|anti_raid
...
class ChannelBot(TimestampHelpersMixin, Base):
	...
	id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
	channel_id: Mapped[int] = mapped_column(ForeignKey("channels.id", ondelete="CASCADE"))
	external_bot_id: Mapped[int] = mapped_column(ForeignKey("external_bots.id", ondelete="CASCADE"))
...
class JoinRequest(CreatedAtMixin, TimestampHelpersMixin, Base):
	...
	__table_args__ = (
		Index("ix_joinreq_channel_user", "channel_id", "user_id"),
	)
	...
	challenge_type: Mapped[str | None] = mapped_column(String(16))  # simple|captcha|keyword
