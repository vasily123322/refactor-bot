...
async def _seed(Session, *, seed: int, threshold: int) -> dict[str, int]:
    async with Session() as session:
        owner = Client(
            ...
            username=f"repeat-views-e2e-v3-{seed}",
            full_name=f"Repeat Views E2E v3 {seed}",
            ui_settings={},
        )
        session.add(owner)
        await session.flush()
        source = Channel(
            ...
            title=f"Repeat Views E2E v3 {seed}",
                ...
                blocks=[
                    {
                        "id": "b1",
                        "type": "text",
                        "text": f"repeat views e2e v3 {seed}",
                    }
                ]
            ),
...
async def _assert_one_pristine_successor(Session, source_id: int, *, threshold: int) -> int:
    ...
    assert AUTODELETE_VIEWS_ACTIONS_META_KEY not in successor_meta
...
def _single_action(meta: dict, message_id: int) -> tuple[dict, dict]:
...
def test_repeat_views_v3_primary_delete_continuation_and_replay_are_single_effect(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-views-e2e-v3.db'}"
        )
        try:
            ...
            async with Session() as session:
                ...
                ledger, action = _single_action(meta, 7601)
                ...
                assert int(action["telegram_message_id"]) == 7601
...
def test_repeat_views_v3_ambiguous_delete_never_replays_or_duplicates_successor(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-views-ambiguous-e2e-v3.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            ...
            async with Session() as session:
                ...
                ledger, action = _single_action(meta, 7701)
                assert int(ledger["threshold"]) == seeded["threshold"]
                assert int(ledger["observed_views"]) == seeded["threshold"] + 10
