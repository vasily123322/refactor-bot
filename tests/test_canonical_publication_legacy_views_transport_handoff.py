...
async def _seed_views_publication(
    Session,
    *,
    seed: int,
    now: datetime,
    ...
    async with Session() as session:
        owner = Client(
            ...
            username=f"views-retirement-{seed}",
            full_name=f"Views Retirement {seed}",
            ui_settings={},
        )
        session.add(owner)
        await session.flush()
        channel = Channel(
            tg_chat_id=-(198100 + seed),
            title=f"Views Retirement {seed}",
        ...
        item = await ContentRepo(session).create(
            ...
            document=PostDocument(
                blocks=[{"id": "b1", "type": "text", "text": f"Views {seed}"}]
            ),
...
def test_views_handoff_is_default_off_then_claims_with_started_capability(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'views-handoff.db'}"
        )
        try:
            ...
            publication_id, task_id = await _seed_views_publication(
                Session,
                seed=1,
                now=now,
            )
            ...
            async with Session() as claim_session:
                claim = await CanonicalPublicationDeliveryCapabilityClaimService(
                    ...
                    holder="canonical-views-primary",
                    ttl_seconds=60,
                    now=now,
                    allow_views_autodelete=True,
                )
                assert claim is not None
                assert claim.plan.runtime_options() == {"autodelete_views": 100}
...
def test_primary_without_started_views_worker_preserves_legacy_claim(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'views-no-worker.db'}"
        )
...
def test_started_views_worker_retires_views_before_legacy_claim(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'views-worker.db'}"
        )
        primary = _PrimaryWorker()
        try:
            async with engine.begin() as connection:
...
def test_views_handoff_rejects_legacy_threshold_drift(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'views-drift.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
...
def test_views_handoff_keeps_effect_compositions_closed(tmp_path) -> None:
    async def assert_ineligible(*, seed: int, runtime_options: dict, suffix: str) -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / f'views-{suffix}.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
    ...
    async def run() -> None:
        await assert_ineligible(
            seed=5,
            runtime_options={"autodelete_views": 100, "pin_on": True},
            suffix="pin-closed",
        )
        await assert_ineligible(
            seed=6,
            runtime_options={"autodelete_views": 100, "autodelete_report": True},
            suffix="report-closed",
        )
        await assert_ineligible(
            seed=7,
            runtime_options={"autodelete_views": 100, "autodelete_seconds": 60},
