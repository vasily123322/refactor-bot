...
async def _seed_terminal(Session, *, seed: int, now: datetime) -> tuple[int, int]:
    async with Session() as session:
        owner = Client(
            tg_user_id=223000 + seed,
            username=f"views-pin-destructive-v3-{seed}",
            full_name=f"Views Pin Destructive v3 {seed}",
            ui_settings={},
        )
        session.add(owner)
        await session.flush()
        channel = Channel(
            ...
            title=f"Views Pin Destructive v3 {seed}",
        ...
        item = await ContentRepo(session).create(
            ...
            document=PostDocument(
                blocks=[{"id": "b1", "type": "text", "text": "views pin destructive v3"}]
            ),
        ...
        await session.commit()
        return int(publication.id), int(message_id)
...
def test_views_pin_destructive_gate_does_not_inherit_plain_repeat_views_fact(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'views-pin-destructive-v3-closed.db'}"
        )
        try:
            ...
            publication_id, _ = await _seed_terminal(Session, seed=1, now=now)
            handle = await _acquire(Session, publication_id, "views-pin-v3-closed")
            views = _Views()
            delete = _Delete()
            async with Session() as session:
            ...
            async with Session() as session:
                ...
                assert AUTODELETE_VIEWS_ACTIONS_META_KEY not in dict(publication.meta or {})
        finally:
...
def test_views_pin_explicit_fact_reuses_v3_reservation_and_no_replay_boundary(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'views-pin-destructive-v3-open.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            Session = async_sessionmaker(engine, expire_on_commit=False)
            now = datetime.now(timezone.utc)
            publication_id, message_id = await _seed_terminal(Session, seed=2, now=now)
            handle = await _acquire(Session, publication_id, "views-pin-v3-open")
            views = _Views()
            delete = _Delete()
            async with Session() as session:
                result = await PublicationAutodeleteViewsComposedService(
            ...
            async with Session() as session:
                ...
                assert int(ledger["threshold"]) == 13
                assert int(ledger["observed_views"]) == 99
                ...
                assert int(action["telegram_message_id"]) == message_id
                assert str(action["reservation_token"])
                assert str(action["autodelete_lease_token"]) == str(handle.lease_token)

            # Keep the original exact lease handle: direct service evaluation does not
            # own/release the worker scheduling lease. Terminal state must still stop the
            # service before any new views or provider call.
            async with Session() as session:
                replay = await PublicationAutodeleteViewsComposedService(
                    ...
                    allow_repeat_views_pin=True,
                    lease=handle,
                ).evaluate_and_delete(publication_id, now=now + timedelta(seconds=30))
