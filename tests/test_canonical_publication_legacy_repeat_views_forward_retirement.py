...
async def _seed(Session, *, seed: int, pin: bool = False) -> tuple[int, int, tuple[int, int]]:
    async with Session() as session:
        owner = Client(
            tg_user_id=217000 + seed,
            username=f"repeat-views-forward-{seed}",
            full_name=f"Repeat Views Forward {seed}",
            ui_settings={},
        )
        session.add(owner)
        await session.flush()
        source = Channel(
            tg_chat_id=-(101817000 + seed), title=f"Views Forward Source {seed}",
            owner_id=int(owner.id), is_active=True,
        )
        first = Channel(
            tg_chat_id=-(101917000 + seed), title=f"Views Forward First {seed}",
            owner_id=int(owner.id), is_active=True,
        )
        second = Channel(
            tg_chat_id=-(102017000 + seed), title=f"Views Forward Second {seed}",
            owner_id=int(owner.id), is_active=True,
        )
        session.add_all([source, first, second])
        await session.flush()
        targets = (int(first.id), int(second.id))
        options = {"autodelete_views": 50, "forward_to": list(targets)}
        if pin:
            options["pin_on"] = True
        item = await ContentRepo(session).create(
            channel_id=int(source.id),
            document=PostDocument(
                blocks=[{"id": "b1", "type": "text", "text": f"Views forward {seed}"}]
            ),
...
def _publish(primary: _PrimaryWorker, *, plain: bool = True, forward: bool = True) -> None:
    set_canonical_publication_delivery_primary_worker(
        primary,
...
def test_views_forward_started_requires_plain_views_and_dedicated_fact() -> None:
    primary = _PrimaryWorker()
    try:
        _publish(primary, plain=False, forward=True)
        assert canonical_publication_delivery_repeat_views_forward_started() is False
        _publish(primary, plain=True, forward=False)
        assert canonical_publication_delivery_repeat_views_forward_started() is False
        _publish(primary)
        assert canonical_publication_delivery_repeat_views_forward_started() is True
    finally:
        set_canonical_publication_delivery_primary_worker(None)


def test_scheduler_yields_exact_ordered_views_forward_then_claims(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-views-forward-yield.db'}"
        )
        primary = _PrimaryWorker()
        try:
            ...
            async with Session() as session:
                insufficient = await CanonicalPublicationLinkedRepeatAtomicHandoffService(
                    ...
                    publication_id,
                    holder="repeat-views-forward-insufficient",
                    ttl_seconds=180,
                    allow_repeat=True,
...
def test_scheduler_keeps_views_forward_on_legacy_when_order_drifts(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-views-forward-drift.db'}"
        )
        primary = _PrimaryWorker()
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            Session = async_sessionmaker(engine, expire_on_commit=False)
            _, task_id, targets = await _seed(Session, seed=2)
...
def test_scheduler_keeps_combined_views_pin_forward_on_legacy(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-views-forward-combined.db'}"
        )
        primary = _PrimaryWorker()
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            Session = async_sessionmaker(engine, expire_on_commit=False)
            _, task_id, _ = await _seed(Session, seed=3, pin=True)
            _publish(primary)
            items = await _mark(Session, task_id)
            assert len(items) == 1 and items[0].status == "processing"
        finally:
