...
async def _seed(Session, *, seed: int) -> tuple[int, int]:
    async with Session() as session:
        owner = Client(
            tg_user_id=240000 + seed,
            username=f"repeat-time-linked-{seed}",
            full_name=f"Repeat Time Linked {seed}",
            ui_settings={},
        )
        session.add(owner)
        await session.flush()
        channel = Channel(
            tg_chat_id=-(100240000 + seed),
            title=f"Repeat Time Linked {seed}",
        ...
        item = await ContentRepo(session).create(
            ...
            document=PostDocument(
                blocks=[{"id": "b1", "type": "text", "text": "repeat time linked"}]
            ),
...
def test_linked_plain_repeat_time_cutover_and_claim_are_one_atomic_transition(
    tmp_path,
) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-time-linked-atomic.db'}"
        )
        try:
...
def test_missing_repeat_time_fact_rolls_prepared_legacy_cutover_back_completely(
    tmp_path,
) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-time-linked-rollback.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            ...
            for seed, allow_time, allow_repeat_time in cases:
...
def test_repeat_time_linked_parity_drift_still_fails_before_authority_transition(
    tmp_path,
) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-time-linked-drift.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            Session = async_sessionmaker(engine, expire_on_commit=False)
            publication_id, task_id = await _seed(Session, seed=4)
