...
async def _seed(Session) -> dict[str, int]:
    async with Session() as session:
        owner = Client(
            tg_user_id=213001,
            username="repeat-pin-forward-e2e",
            full_name="Repeat Pin Forward E2E",
            ui_settings={},
        )
        session.add(owner)
        await session.flush()
        source = Channel(
            tg_chat_id=-100213001,
            title="Repeat Pin Forward Source",
            owner_id=int(owner.id),
            is_active=True,
        )
        target_a = Channel(
            tg_chat_id=-100313001,
            title="Repeat Pin Forward A",
            owner_id=int(owner.id),
            is_active=True,
        )
        target_b = Channel(
            tg_chat_id=-100413001,
            title="Repeat Pin Forward B",
            owner_id=int(owner.id),
            is_active=True,
        )
...
class _PinForwardBot:
    def __init__(self) -> None:
        self.events: list[dict] = []

    async def pin_chat_message(self, **kwargs) -> None:
        self.events.append({"kind": "pin", **dict(kwargs)})

    async def forward_message(self, **kwargs) -> None:
        self.events.append({"kind": "forward", **dict(kwargs)})
...
def test_repeat_pin_forward_publish_continuation_and_replay_are_single_effect(
    tmp_path,
) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-pin-forward-e2e.db'}"
        )
        try:
            ...
            async with Session() as session:
                ...
                assert sum(action.action_type == "pin" for action in actions) == 1
                assert sum(action.action_type == "forward" for action in actions) == 4
