...
async def _seed_terminal(Session, *, seed: int, now: datetime) -> int:
    async with Session() as session:
        owner = Client(
            ...
            username=f"repeat-time-destructive-{seed}",
            full_name=f"Repeat Time Destructive {seed}",
            ui_settings={},
        )
        session.add(owner)
        await session.flush()
        channel = Channel(
            ...
            title=f"Repeat Time Destructive {seed}",
        ...
        item = await ContentRepo(session).create(
            ...
            document=PostDocument(
                blocks=[
                    {"id": "b1", "type": "text", "text": "repeat time destructive"}
                ]
            ),
        ...
        publication = await LegacyPublicationBridge(session).queue(
            ...
            runtime_options={"silent": True, "autodelete_seconds": 90},
        )
        ...
        publication.telegram_message_ids = [8800 + seed]
        ...
        session.add(
            PublicationAttempt(
                ...
                attempt=1,
                status="published",
                telegram_message_ids=[8800 + seed],
                error=None,
...
class _Provider:
    ...
    async def delete_message(self, *, chat_id: int, message_id: int):
        # Reservation must be committed before the irreversible provider boundary.
...
def test_repeat_time_destructive_admission_is_default_off(tmp_path) -> None:
    async def run() -> None:
        now = datetime.now(timezone.utc)
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-time-destructive-off.db'}"
        )
        try:
...
def test_repeat_time_delete_uses_exact_281_ledger_and_replay_calls_provider_zero_times(
    tmp_path,
) -> None:
    async def run() -> None:
        now = datetime.now(timezone.utc)
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-time-destructive-success.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
...
def test_repeat_time_ambiguous_delete_is_permanent_no_replay_barrier(tmp_path) -> None:
    async def run() -> None:
        now = datetime.now(timezone.utc)
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-time-destructive-ambiguous.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            ...
            async with Session() as session:
                ...
                first = await CanonicalRepeatTimeAutodeleteService(
                    session,
                    provider=provider,
                    allow_repeat_time=True,
                ).delete_if_due(publication_id, now=now)
...
def test_repeat_time_lifecycle_drift_blocks_reservation_and_provider(tmp_path) -> None:
    async def run() -> None:
        now = datetime.now(timezone.utc)
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'repeat-time-destructive-drift.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
