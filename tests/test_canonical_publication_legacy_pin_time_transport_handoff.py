...
async def _seed_pin_time_publication(
    Session,
    *,
    seed: int,
    now: datetime,
    ...
    async with Session() as session:
        owner = Client(
            ...
            username=f"pin-time-retirement-{seed}",
            full_name=f"Pin Time Retirement {seed}",
            ui_settings={},
        )
        session.add(owner)
        await session.flush()
        channel = Channel(
            tg_chat_id=-(200100 + seed),
            title=f"Pin Time Retirement {seed}",
        ...
        item = await ContentRepo(session).create(
            ...
            document=PostDocument(
                blocks=[
                    {"id": "b1", "type": "text", "text": f"Pin Time {seed}"}
                ]
            ),
        ...
        publication = await LegacyPublicationBridge(session).queue(
            ...
            runtime_options=(
                {"pin_on": True, "autodelete_seconds": 60}
...
def test_pin_time_handoff_requires_time_fact_then_canonical_claims(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'pin-time-handoff.db'}"
        )
        try:
            ...
            publication_id, task_id = await _seed_pin_time_publication(
                Session,
                seed=1,
                now=now,
            )
            ...
            async with Session() as claim_session:
                claim = await CanonicalPublicationDeliveryCapabilityClaimService(
                    ...
                    holder="canonical-pin-time-primary",
                    ttl_seconds=60,
                    now=now,
                    allow_time_autodelete=True,
                )
...
def test_primary_without_time_worker_preserves_pin_time_legacy_claim(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'pin-time-no-worker.db'}"
        )
...
def test_started_time_worker_retires_pin_time_before_legacy_claim(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'pin-time-worker.db'}"
        )
        primary = _PrimaryWorker()
        try:
            async with engine.begin() as connection:
...
def test_pin_time_legacy_pin_drift_stays_legacy_owned(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'pin-time-drift.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
...
def test_pin_time_keeps_report_and_pin_views_closed(tmp_path) -> None:
    async def assert_ineligible(*, seed: int, runtime_options: dict, suffix: str) -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / f'pin-time-{suffix}.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
