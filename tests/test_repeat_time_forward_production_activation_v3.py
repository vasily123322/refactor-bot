...
def test_safe_runtime_time_forward_requires_plain_time_and_its_own_fact() -> None:
    ...
    assert enabled.executor.allow_repeat_time_forward is True
    assert enabled.executor.allow_repeat_time_pin is False
    ...
    for kwargs in missing:
        ...
        assert runtime.executor.allow_repeat_time_forward is False


def test_primary_control_never_infers_time_forward_from_plain_or_pin(monkeypatch) -> None:
    async def run() -> None:
        ...
        assert captured[-1]["allow_repeat_time_forward"] is False
        ...
        assert captured[-1]["allow_repeat_time_pin"] is False
        ...
        await control.start_canonical_publication_safe_repeat_primary_if_enabled(
            ...
            time_autodelete_executor_available=True,
            repeat_continuation_available=True,
            repeat_time_executor_available=False,
            repeat_time_forward_executor_available=True,
            repeat_owner_policy_enforced=True,
        )
        assert captured[-1]["allow_repeat_time_forward"] is False
...
def test_dispatcher_time_forward_worker_requires_started_plain_time_and_forwards_exact_fact(
    monkeypatch,
) -> None:
    async def run() -> None:
        ...
        class FakeTimeForwardWorker:
            def __init__(self, **kwargs) -> None:
                events.append("construct")
                assert kwargs["allow_repeat_time_forward"] is True
                self.repeat_time_forward_available = False

            async def start(self) -> None:
                events.append("start")
                self.repeat_time_forward_available = True
        ...
        monkeypatch.setattr(
            ...
            FakeTimeForwardWorker,
        )

        assert (
            await dispatcher._start_canonical_repeat_time_forward_autodelete_worker_if_enabled(
        ...
        worker = await dispatcher._start_canonical_repeat_time_forward_autodelete_worker_if_enabled(
            repeat_continuation_available=True,
            repeat_time_available=True,
        )
        assert worker is not None
        assert worker.repeat_time_forward_available is True
        ...
        result = await dispatcher._start_canonical_publication_delivery_workers(
            ...
            repeat_time_executor_available=True,
            repeat_time_forward_executor_available=worker.repeat_time_forward_available,
        )
        ...
        assert captured["repeat_time_forward_executor_available"] is True
