...
def test_repeat_views_and_unknown_runtime_remain_fail_closed(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
        ...
        try:
            ...
            unknown_id = await _seed_published(
                Session,
                seed=5,
                scheduled_at=scheduled_at,
                delivered_at=delivered_at,
            ...
            async with Session() as session:
                ...
                assert await planner.plan(unknown_id) is None
        finally:
            await engine.dispose()
