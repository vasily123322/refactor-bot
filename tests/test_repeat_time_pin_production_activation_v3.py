...
def test_safe_runtime_time_pin_requires_plain_time_and_its_own_fact() -> None:
    ...
    assert enabled.executor.allow_repeat_time_pin is True
    ...
    for kwargs in missing:
        ...
        assert runtime.executor.allow_repeat_time_pin is False


def test_primary_control_never_infers_time_pin_from_plain_time(monkeypatch) -> None:
    async def run() -> None:
...
def test_dispatcher_time_pin_worker_requires_started_plain_time_and_forwards_exact_fact(
    monkeypatch,
) -> None:
    async def run() -> None:
        ...
        class FakeTimePinWorker:
            def __init__(self, **kwargs) -> None:
                events.append("construct")
                assert kwargs["allow_repeat_time_pin"] is True
                self.repeat_time_pin_available = False

            async def start(self) -> None:
                events.append("start")
                self.repeat_time_pin_available = True
        ...
        assert (
            await dispatcher._start_canonical_repeat_time_pin_autodelete_worker_if_enabled(
        ...
        worker = await dispatcher._start_canonical_repeat_time_pin_autodelete_worker_if_enabled(
            repeat_continuation_available=True,
            repeat_time_available=True,
        )
        assert worker is not None
        assert worker.repeat_time_pin_available is True
        ...
        result = await dispatcher._start_canonical_publication_delivery_workers(
            ...
            repeat_time_executor_available=True,
            repeat_time_pin_executor_available=worker.repeat_time_pin_available,
        )
        ...
        assert captured["repeat_time_pin_executor_available"] is True
