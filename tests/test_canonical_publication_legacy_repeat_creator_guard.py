...
async def _rows(Session) -> list[PostTask]:
    async with Session() as session:
        return list((await session.execute(select(PostTask).order_by(PostTask.id))).scalars().all())
...
def test_boot_cleanup_never_creates_or_skips_canonical_owned_repeat(tmp_path, monkeypatch) -> None:
    async def run() -> None:
        engine = create_async_engine(f"sqlite+aiosqlite:///{tmp_path / 'repeat-boot-guard.db'}")
        try:
...
def test_overflow_never_mutates_canonical_owned_repeat_group(tmp_path, monkeypatch) -> None:
    async def run() -> None:
        engine = create_async_engine(f"sqlite+aiosqlite:///{tmp_path / 'repeat-overflow-guard.db'}")
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            Session = async_sessionmaker(engine, expire_on_commit=False)
            ...
            assert all(dict(row.payload or {}).get("repeat_on") is True for row in rows)
        finally:
            await engine.dispose()
...
def test_unproven_repeat_still_uses_legacy_boot_cleanup(tmp_path, monkeypatch) -> None:
    async def run() -> None:
        engine = create_async_engine(f"sqlite+aiosqlite:///{tmp_path / 'repeat-boot-fallback.db'}")
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            Session = async_sessionmaker(engine, expire_on_commit=False)
            [task_id] = await _seed(Session, seed=3)
            monkeypatch.setattr(
                "app.workers.canonical_repeat_continuation_scheduler."
                "canonical_publication_delivery_repeat_started",
                lambda: True,
            ...
            monkeypatch.setattr(Scheduler, "_yield_proven_repeat_to_canonical_primary", not_proven)
            scheduler = Scheduler(Session, SimpleNamespace(), repeat_continuation_enabled=False)
            scheduler._boot_time = datetime.now(timezone.utc)
            scheduler._boot_cleanup_done = False
            async with Session() as session:
