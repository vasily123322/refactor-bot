...
async def _seed(
    ...
    *,
    ...
    async with Session() as session:
        ...
        # Most evaluator tests deliberately exercise the linked compatibility path,
        # including threshold races against the transport payload. Plain views intent
        # is now canonical/PostTask-free, so create that compatibility transport only
        # for linked scenarios. Explicit unlink=True remains truly PostTask-free.
        if task is None and not unlink:
            task = PostTask(
                channel_id=int(channel.id),
                status="pending",
                payload=dict(runtime_options),
                dedupe_key=f"test-views-evaluator-compat:{int(publication.id)}",
