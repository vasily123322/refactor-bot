...
async def _seed_terminal(Session, *, seed: int, now: datetime) -> int:
    async with Session() as session:
        owner = Client(
            ...
            username=f"combined-destructive-{seed}",
            full_name=f"Combined Destructive {seed}",
            ui_settings={},
        )
        session.add(owner)
        await session.flush()
        source = Channel(
            ...
            title=f"Combined Destructive Source {seed}",
            owner_id=int(owner.id),
            is_active=True,
        )
        target_a = Channel(
            tg_chat_id=-(100337000 + seed),
            title=f"Combined Destructive A {seed}",
            owner_id=int(owner.id),
            is_active=True,
        )
        target_b = Channel(
            tg_chat_id=-(100437000 + seed),
            title=f"Combined Destructive B {seed}",
            owner_id=int(owner.id),
            is_active=True,
        )
        ...
        item = await ContentRepo(session).create(
            ...
            document=PostDocument(
                blocks=[{"id": "b1", "type": "text", "text": "combined destructive"}]
            ),
        ...
        publication.telegram_message_ids = [9800 + seed]
        ...
        session.add(
            PublicationAttempt(
                ...
                attempt=1,
                status="published",
                telegram_message_ids=[9800 + seed],
                error=None,
...
def test_independent_pin_forward_destructive_facts_do_not_imply_combined(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'combined-destructive-closed.db'}"
        )
        try:
            ...
            handle = await _acquire(Session, publication_id, "combined-closed")
            views = _Views()
            delete = _Delete()
            async with Session() as session:
...
def test_exact_combined_destructive_fact_reuses_current_v3_reservation_boundary(tmp_path) -> None:
    async def run() -> None:
        engine = create_async_engine(
            f"sqlite+aiosqlite:///{tmp_path / 'combined-destructive-open.db'}"
        )
        try:
            async with engine.begin() as connection:
                await connection.run_sync(Base.metadata.create_all)
            ...
            handle = await _acquire(Session, publication_id, "combined-open")
            views = _Views()
            delete = _Delete()
